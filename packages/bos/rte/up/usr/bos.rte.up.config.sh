# @(#)61        1.4  src/packages/bos/rte/up/usr/bos.rte.up.config.sh, pkgbos, pkg411, 9439B411c 9/29/94 12:46:45
#
...
# The links should only be added if this is a UP system.

RV=`bootinfo -z`
if [ "$RV" -eq 0 ]
then
	ln -fs /usr/lib/boot/unix_up /usr/lib/boot/unix || exit 1
        exit 0
fi
