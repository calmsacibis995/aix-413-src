#!/bin/ksh
#  @(#)80        1.4  src/packages/X11/fnt/iso8/usr/X11.fnt.iso8.post_i.sh, pkgx, pkg411, GOLD410 3/8/94 15:37:13
#
#   COMPONENT_NAME: pkgx
#
#   FUNCTIONS: X11.fnt.iso8 (usr) post installation configuration
#
#   ORIGINS: 27
#
#
...
#----------------------------------------------------------------
#   Add iso8 entries to X11 i18n fonts.alias
#----------------------------------------------------------------
if [ ! -f $FONTDIR/fonts.alias ]; then
  echo "FILE_NAMES_ALIASES" > $FONTDIR/fonts.alias || exit 1
fi
cat $FONTDIR/fonts.alias.iso8 >> $FONTDIR/fonts.alias || exit 1
