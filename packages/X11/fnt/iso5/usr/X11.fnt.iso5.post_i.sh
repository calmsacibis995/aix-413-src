#!/bin/ksh
#  @(#)76        1.5  src/packages/X11/fnt/iso5/usr/X11.fnt.iso5.post_i.sh, pkgx, pkg411, GOLD410 3/8/94 15:40:53
#
#   COMPONENT_NAME: pkgx
#
#   FUNCTIONS: X11.fnt.iso5 (usr) post installation configuration
#
#   ORIGINS: 27
#
#
...
#----------------------------------------------------------------
#   Add iso5 entries to X11 i18n fonts.alias
#----------------------------------------------------------------
if [ ! -f $FONTDIR/fonts.alias ]; then
  echo "FILE_NAMES_ALIASES" > $FONTDIR/fonts.alias || exit 1
fi
cat $FONTDIR/fonts.alias.iso5 >> $FONTDIR/fonts.alias || exit 1
