#!/bin/ksh
#  @(#)82        1.4  src/packages/X11/fnt/iso9/usr/X11.fnt.iso9.post_i.sh, pkgx, pkg411, GOLD410 3/8/94 15:38:50
#
#   COMPONENT_NAME: pkgx
#
#   FUNCTIONS: X11.fnt.iso9 (usr) post installation configuration
#
#   ORIGINS: 27
#
#
...
#----------------------------------------------------------------
#   Add iso9 entries to X11 i18n fonts.alias
#----------------------------------------------------------------
if [ ! -f $FONTDIR/fonts.alias ]; then
  echo "FILE_NAMES_ALIASES" > $FONTDIR/fonts.alias || exit 1
fi
cat $FONTDIR/fonts.alias.iso9 >> $FONTDIR/fonts.alias || exit 1
