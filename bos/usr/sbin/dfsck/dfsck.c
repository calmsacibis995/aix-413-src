static char sccsid[] = "@(#)61	1.15.2.2  src/bos/usr/sbin/dfsck/dfsck.c, cmdfs, bos411, 9428A410j 1/10/94 17:56:26";
/*
...
/* DFSCK                                                        */
/*                                                              */
/*      This module performs 2 fsck's simultaneously.           */


#include <nl_types.h>
#include "dfsck_msg.h"
#define MSGSTR(N,S)	catgets(catd,MS_DFSCK,N,S)
nl_catd catd;
...
#define MAXFDS  200              /* max open files */
...
char **argv;
{
	...
	{	fprintf( stderr, MSGSTR(USAGE, use), argv[0]);
		exit( 1);
	}
	...
	if(!ans && !bd2flg) {		/* identify file systems */
		printf(MSGSTR(ONEWILL, "1 will identify "));
		for ( i = arg1; i < opt2; i++ ) printf( "%s ", argv[i]);
		printf(MSGSTR(TWOWILL, "\n2 will identify "));
		for ( i = arg2; i < argc; i++ ) printf( "%s ", argv[i]);
		printf(MSGSTR(PRECED1, "\nPrecede every answer with 1 or 2\n"));
		printf(MSGSTR(ASIN1, "as in `1y' for %s\n"),argv[arg1]);
	}

			/* pipe set-up */
	if(pipe(p1fs1) == -1)
		printf(MSGSTR(CANTPIPE1, "Cannot open pipe for first fsck\n"));
	if(pipe(p2fs1) == -1)
		printf(MSGSTR(CANTPIPE2, "Cannot open pipes for first fsck\n"));
	ftendi1 = p1fs1[0];
	ftendo1 = p2fs1[1];
	fsout1= p1fs1[1];
	fsin1 = p2fs1[0];
	...
	if(pipe(p1fs2) == -1)
		printf(MSGSTR(CANTPIPE3, "Cannot open pipe for second fsck\n"));
	if(pipe(p2fs2) == -1)
		printf(MSGSTR(CANTPIPE4, "Cannot open pipes for second fsck\n"));
	ftendi2 = p1fs2[0];
	ftendo2 = p2fs2[1];
	fsout2 = p1fs2[1];
	fsin2 = p2fs2[0];
	...
	case 0:			/* child */
		...
		close(ftendo1);
		for(i = 3; i < MAXFDS; i++)	close(i);
		execvp( "/usr/sbin/fsck", argv);
		printf(MSGSTR(CANTEXEC, "Can't exec 'fsck' on %s\n"),argv[arg1]);
		exit(2);
	case -1:		/* unsuccessful fork */
		printf(MSGSTR(CANTFORK, "Couldn't fork on %s\n"),argv[arg1]);
		exit(2);
	default:		/* parent */
		close(fsin1);
		close(fsout1);
	...
	setdlys();		/* set delays */
	signal(SIGPIPE,(void (*)(int))sigcatch);
	...
	for(;;) {
		...
		if((pid = wait(&status)) != -1) {
			alarm(0);
			if (pid == pid1) estat1 = (status >> 8);
			else estat2 = (status >> 8);
			if(pid == pid1 || pid == pid2) {
				if(stat = status & 0377) {  /* signal detected */
					if(status & 0200)
						printf(MSGSTR(CORED, "core dumped\n"));
					printf(MSGSTR(CAUGHT, "signal %o caught\n"),stat);
			   ...
			   printf(MSGSTR(UNKNCHD, "Unknown child fdes\n"));
			   exit(2);
			}
		}
		...
		while ((read(ttyi,&buf2,1)) > 0 ) {
			...
			if((!bd2flg && !two) && (!bd1flg && !one) && !ans) {
				...
				len = strlen((strptr = MSGSTR(QUEST, "which filesystem? answer '1' or '2'\n")));
				write(ttyio, strptr, len);
				c = buf2;
				ans++;
			}
			if(one) {
		...
		if((exit1 && exit2) || (bd2flg && exit1)) {
			printf(MSGSTR(DDONE, ">>> DFSCK DONE <<<\n"));
			break;
		}
	}
	return (estat1 ? estat1 : estat2);
}
...
int ofd,nfd;
{
	...
	if(fcntl(ofd,F_DUPFD,nfd) == -1) {
		printf(MSGSTR(CANTCHANG, "Cannot change file descriptor\n"));
		exit(2);
	}
	close(ofd);
	return;
...
sigcatch(int sig)
{
	...
	len = strlen ((strptr = MSGSTR(PIPERROR, "pipe error: write to wrong pipe(fsck)\n")));
...
setdlys()
{
	...
	if((ttyi = fcntl(ttyio,F_DUPFD,0)) == -1) {
		printf(MSGSTR(CANTDUP, "Cannot dup tty fdes for ttyi\n"));
		exit(2);
	}
	close(1);
	if((ttyo = fcntl(ttyio,F_DUPFD,1)) == -1) {
		printf(MSGSTR(CANTDUP1, "Cannot dup tty fdes\n"));
		exit(2);
	}
	if((fcntl(ttyi,F_SETFL,O_NDELAY)) == -1) {
		printf(MSGSTR(CANTSET, "Couldn't set O_NDELAY on ttyi\n"));
		exit(2);
	}
