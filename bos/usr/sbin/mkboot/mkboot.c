static char sccsid[] = "@(#)62 1.35.1.15 src/bos/usr/sbin/mkboot/mkboot.c, bosboot, bos41J, 9517B_all 95/04/26 18:19:14";
/*
 ...
 * NAME:	mkboot.c
 *
 * FUNCTION:	This low level command creates a boot image file
 *		for a specified device. A boot image file
 *		consists of a kernel, RAM proto file system,
 *		and optionally a boot and config record. The ROS
 *		IPL code interprets the boot and config record
 *		information so as to load the boot image file into
 *		RAM and pass control to it.
 *
...
main (int argc, char **argv)
{
	...
	switch(flg) {
		case 'A':
			...
			break;
		case 'b':
			no_savebase = 1;	/* no savebase on image */
			break;
		case 'c':
			clear_boot = 1;		/* zero out boot record */
			break;
		case 'D':
			mk_debug = DONT_TRAP;	/* load low level debugger */
			break;
		case 'd':
			...
			break;
		case 'e':
			kernel = optarg;	/* expand code for kernel */
			if (kflag) errflg++; else eflag++;
			break;
		case 'F':
			aixmon_dbg=TRUE;
			break;
		...
		case 'h':
			...
			break;
		case 'I':
			mk_debug = DO_TRAP;	/* invoke low level debugger */
			break;
		case 'i':
		...
		case 'k':
			...
			break;
		case 'L':
			lock_instrumentation = TRUE;
			break;
		case 'l':
			lv_dev = optarg;	/* boot physical partition */
			break;
		case 'm':			/* Setup boot mode */
			mode_override=TRUE;
			mode_string = optarg;
			break;
		case 'p':
			boot_start_loc = atoi(optarg);
			break;
		case 'r':
			ros_emulation = 1;	/* image is ROS Emulation */
			break;
		case 's':
			write_serv_rec = 1;	/* create service boot record */
			break;
		case 'w':
	...
	if (rspc) {
		if (mode_override) {		/* mode_override is true */
			int i;
			for(i=0; i<strlen(mode_string); i++)
				switch(mode_string[i]) {
					case    'c':
					case    'C':
					mode_control &= 0xFFFFFF0F;
					break;

					case    'l':
					case    'L':
					mode_control |= LODI_MODE;
					break;

					case    'j':
					case    'J':
					mode_control |= JUMPI_MODE;
					break;

					default:
					mkboot_error(1, execname, NULL, 0);
					break;
				}
		}
		else if (!aixmon_dbg) {
			/*
			 * mode_override is false and aixmon_dbg is false
			 *
			 * set defaults
			 * Don't do this for aixmon_dbg because boot will
			 * hang with nothing to jump to
			 */
			mode_control |= LODI_MODE;
			mode_control |= JUMPI_MODE;
		}

		/* check for power management disable switch */
		pmmi_data_CLASS = odm_mount_class("pmmi_data");
		rc = (int)odm_get_first(pmmi_data_CLASS,
			"attribute = 'disable_hiber'", (void *)0);
		if ((rc != 0) && (rc != -1))
			/*
			 * The entry was found, so hibernation is to be
			 * disabled at next boot. Set a bit to serve as a flag.
			 *
			 * The only bits in use in mode_control at this time
			 * are LODI_MODE (0x40), JUMPI_MODE (0x80) and 0x1.
			 */
			mode_control |= 0x1;
	...
	if (clear_boot) {
		/*
		 * clear_rec declared static (all zeros).
		 * Save physical volume id.
		 */
		pv_id_save = boot_rec.pv_id;
		IPL_record_id_save = boot_rec.IPL_record_id;
		boot_rec = clear_rec;
		boot_rec.pv_id = pv_id_save;
		/*
		 * this is a future fix to prevent the IPL_record_id from
		 * always being changed to IPLRECID - see defect 170051
		 */
		/* boot_rec.IPL_record_id = IPL_record_id_save; */
		boot_rec.IPL_record_id = IPLRECID;
		write_disk_record(bdevice);
		exit(0);
	}
	...
	if (rspc) {
		if ((aixmon_fd = fopen(aixmon_name, "rb" )) == NULL)
			mkboot_error(4, execname, aixmon_name, 0);
	}
	...
	if (rspc) {
		int i;
		if(fstat(fileno(aixmon_fd),&sbuf1)) {
			mkboot_error(4, execname, aixmon_name, 0);
		}

		/*
		 * read the XTOC from aixmon and save the entry point
		 */
		read_block(aixmon_fd, UBSIZE, &aixmon_read_buffer);
		if (A_XTOC.filehdr.f_magic == RS_TOC_NUM ||
			A_XTOC.filehdr.f_magic == OLD_TOC_NUM ) {
			/* save the aixmon entry_point */
			aixmon_ep =
				A_XTOC.scnhdr[A_XTOC.aouthdr.o_sntext-1].s_scnptr;
		}
		else	/* not TOC magic number */
			mkboot_error(8, execname, aixmon_name, 0);

		/* initialize the rspc bootrecord */
		init_boot_partition_table(&p_table);

		/* Fill in the hints_data section */
		init_hints_table(&hints_data);

		/* Fill in the partition header section */

		p_head.entry_offset = swap_endian(1024 +
				(unsigned)sizeof(hints_data));

		/*
		 * ROS wants the prog_length field to be a multiple of 4
		 * so that's why we add 3 and mask off the last three bits
		 */
		if (aixmon_dbg)
			p_head.prog_length = swap_endian((hints_data.header_size
				+ sbuf1.st_size + 3) & 0xFFFFFFFC);
		else
			p_head.prog_length =
				swap_endian(((hints_data.header_block_size
					* UBSIZE) + file_size + 3)
						& 0xFFFFFFFC);

		/* malloc a buffer for building rspc specific boot code */
		if ((masters_pimage =
			(char *)malloc(hints_data.header_block_size * UBSIZE))
				== NULL)
			mkboot_error(7, execname, NULL,
				hints_data.header_block_size * UBSIZE);


		tot_bytes = 0;

		/* first write the partition header to the malloc'ed block */
		memcpy(&masters_pimage[tot_bytes], &p_head,
			sizeof(partition_header));
		tot_bytes += sizeof(partition_header);

		/* next write the (unused) data block to the malloc'ed block */
		for (i=0;i<DATA_BLOCK_LEN;i++)
			masters_pimage[tot_bytes++] = 0;

		/* next write the hints_data to the malloc'ed block */
		memcpy(&masters_pimage[tot_bytes], &hints_data,
			sizeof(hints_data));
		tot_bytes += sizeof(hints_data);

		/* skip over the XCOFF header in the aixmon file */
		fseek(aixmon_fd,aixmon_ep, SEEK_SET);

		/* now write the aixmon file to the malloc'ed block */
		while (!feof(aixmon_fd)) {
			rc = fread(xfer_buf,1,XFER_BUF_SIZE,aixmon_fd);
			if (rc > 0) {
				memcpy(&masters_pimage[tot_bytes],xfer_buf,rc);
				tot_bytes += rc;
			}
		}
		...
		/* last step is to write some padding to the malloc'ed block */
		while(tot_bytes % 512) {
			/* round up to next whole sector */
			masters_pimage[tot_bytes] = 'X';
			tot_bytes++;
		}
	}
	else {
	...
	if (rspc) {
		...
		fwrite(masters_pimage,1,tot_bytes,out_file);

		/* Finished if this is aixmon debug */
		if(aixmon_dbg) {
			fclose(out_file);
			exit(0);
		}
	...
	if (no_boot_hdr == 0) {
	   ...
	   *
	   */
	  /* Initialize boot header buffer with data */
	  boot_header[0] = 0;			/* trap instruction */
	  boot_header[1] = 0xBADFCA11;		/* header size */
	  boot_header[2] = file_pos.end_of_text - text_start; /* headr offset */
	  boot_header[3] = file_pos.start_disk;	/* RAM disk start */
	  boot_header[4] = file_pos.end_disk;	/* RAM disk end */
	  boot_header[5] = mk_debug;		/* LL debugger availability */
	  if (no_savebase) {
		boot_header[6] = SAVE_ZERO; /* RAM savebase start */
		boot_header[7] = SAVE_ZERO; /* RAM savebase end */
		boot_header[8] = SAVE_ZERO; /* RAM savebase disk address */
	  }
	  else {
		boot_header[6] = file_pos.end_disk;
		boot_header[7] = file_size;
		boot_header[8] = boot_rec.boot_prg_start +
			(number_of_sectors_calculated - (SBSIZE / UBSIZE)) +
			hints_data.header_block_size;
	  }

	  /*
	   *
	   * Find start of boot header which follows the XTOC.
	   *
	   */
	  /* Boot header not in first XTOC block (large kernel). */
	  if (text_start >= UBSIZE) {
		/* skip blocks until header */
		num_blks = text_start / UBSIZE;
		for (cntr=0;cntr < num_blks;cntr++) {
			write_block(out_file, UBSIZE, &kernel_read_buffer);
			read_block(in_file, UBSIZE, &kernel_read_buffer);
		}
	  }
	  /* Boot header block found. */
	  if ((text_start + kernel_ep) < (UBSIZE * (num_blks + 1))) {
		b_index = (text_start / LONG_BYTES);
		/* correct b_index for number of blocks skipped */
		b_index -= UB_LONGS * num_blks;
		for (cntr=0;cntr < BH_LONGS;cntr++)
			kernel_read_buffer.buf_longs[b_index++] =
							boot_header[cntr];
	  }
	  else {
		cntr = 0;
		...
		/* correct b_index for number of blocks skipped */
		b_index -= UB_LONGS * num_blks;
		if (text_start < (UBSIZE * (num_blks + 1))) {
			/* split - crosses 512 byte block boundary */
			for (b_index=b_index;b_index < UB_LONGS && cntr < BH_LONGS;b_index++) {
				kernel_read_buffer.buf_longs[b_index] =
							boot_header[cntr++];
			}
			b_index = 0;
		...
		/* Either write whole boot header or finish split header. */
		for (cntr=cntr;cntr < BH_LONGS;cntr++) {
			kernel_read_buffer.buf_longs[b_index++] =
							boot_header[cntr];
		}
	  }
...

{
	...
	if (rspc) {
		p_table.partition[0].RBA = swap_endian(pbn + 2);
		p_table.partition[0].sectors =
				swap_endian(hints_data.header_block_size);
		memcpy((char *)&boot_rec + PART_START, &p_table,
				(sizeof(p_table)));
		((char *)&boot_rec)[SIG_START] = 0x55;
		((char *)&boot_rec)[SIG_START + 1] = 0xAA;
	} /* end of normal boot record initialization */
...

{
	...
	if (rspc) {
		p_table.partition[1].RBA = swap_endian(pbn + 2);
		p_table.partition[1].sectors =
				swap_endian(hints_data.header_block_size);
		memcpy((char *)&boot_rec + PART_START, &p_table,
				(sizeof(p_table)));
		((char *)&boot_rec)[SIG_START] = 0x55;
		((char *)&boot_rec)[SIG_START + 1] = 0xAA;
	} /* end of service boot record initialization */
