static char sccsid[] = "@(#)21  1.2.1.4  src/bos/usr/sbin/install/sysck/syssrc.c, cmdinstl, bos411, 9428A410j 1/15/94 17:53:42";
/*
 * COMPONENT_NAME: cmdinstl
 *
 * FUNCTIONS: cp_source, ck_source, src_file, src_char_dev
 *            src_blk_dev, src_mpx_dev, src_directory
 *            src_fifo, cp_source
...
ck_source (struct tcbent *tcbent)
{
	 ...
	 */
        ...
        MSG_S(MSG_Wrong_File_Type,DEF_Wrong_File_Type,src,0,0,0);
	return -1;
}
...
src_char_dev (struct tcbent *tcbent)
{
	...
	if (*src == '\0') {
	        MSG_S(MSG_Donot_Know_How,DEF_Donot_Know_How,dst,0,0,0);
		return -1;
	}
...
cp_source (struct tcbent *tcbent)
{
	...
	if (! exist && (tcbent->tcb_owner == (uid_t) -1 ||
		...
		MSG_S(MSG_Needs_An_Attribute,DEF_Needs_An_Attribute,"owner",dst,0,0);

		if (tcbent->tcb_group == (gid_t) -1)
		MSG_S(MSG_Needs_An_Attribute,DEF_Needs_An_Attribute,"group",dst,0,0);

		if (tcbent->tcb_mode == (mode_t) -1 && ! tcbent->tcb_acl)
		MSG_S(MSG_Needs_An_Attribute,DEF_Needs_An_Attribute,"mode",dst,0,0);
