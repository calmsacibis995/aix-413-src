static char sccsid[] = "@(#)90	1.7.2.2  src/bos/usr/sbin/lvm/lresynclv.c, cmdlvm, bos411, 9428A410j 1/18/94 10:06:08";
 ...
 * FILE: lresynclv.c
 *
 * FILE DESCRIPTION: Source file for lresynclv program.  "lresynclv" provides
 *     a command level user interface to the lvm low level function lvm_resynclv.
 *
...
char **argv;	 /* array of command line strings */
{
	...
	while ((rc = getopt(argc, argv, "fl:")) != EOF) {
		switch (rc) {
		case 'f':
			f_flag = 1;
			break;
...
int Usage_error()
{
	fprintf(stderr, lvm_msg(RSLV_USAGE), Prog);
}
 ...
 * NAME: main
 *
 * FUNCTION: lresynclv reads command line input from the user,
 *    parses it, performs basic validation, formats it, and calls
 *    lvm_resynclv passing the formatted values.  If an error is
...
char **argv;	   /* array of command line strings - from user */
{
	...
	if ((retval=ck_required()) == 0) {  /* ck_required confirms required parameters were supplied */
		...
		else force = FALSE;  /* default is false, no resync of non-stale pps */

		/* call lvm to resync the logical volume */
		if (retval = lvm_resynclv(&lv_id,force)) {
			errmsg(retval);
			exit(lvm_rc(retval));
		}
	}
