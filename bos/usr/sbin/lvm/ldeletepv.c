static char sccsid[] = "@(#)20	1.9.1.1  src/bos/usr/sbin/lvm/ldeletepv.c, cmdlvm, bos411, 9428A410j 1/18/94 10:05:41";
 ...
 * FILE: ldeletepv.c
 *
 * FILE DESCRIPTION: Source file for ldeletepv program.  "ldeletepv" provides
 *     a command level user interface to the lvm low level function lvm_deletepv.
 *
...
/* The flag corresponds to a command options - flag true if option specified on command line */
static int g_flag;
static int p_flag;
...
char **argv;	 /* array of command line strings */
{
	...
	while ((rc = getopt(argc, argv, "g:p:")) != EOF) {
		switch (rc) {
		case 'g':
			g_flag = 1;
			g_value = optarg;
...
int Usage_error()
{
	fprintf(stderr, lvm_msg(DPV_USAGE), Prog);
}
 ...
 * NAME: main
 *
 * FUNCTION: ldeletepv reads command line input from the user,
 *    parses it, performs basic validation, formats it, and calls
 *    lvm_deletepv passing the formatted values.  If an error is
 *    encountered then a message is displayed.
 *
 * RETURN VALUE DESCRIPTION: Zero or positive if no error. Negative otherwise.
 */
...
char **argv;	   /* array of command line strings - from user */
{
	...
	struct unique_id vg_id,pv_id;  /* volume group id and physical value id */
	...
	if ((retval=ck_required()) == 0) {  /* ck_required confirms required parameters were supplied */
		...
		/* format input values pass to lvm_deletepv to delete the pv from the vg */
		...
		if (retval = lvm_deletepv(&vg_id,&pv_id)) {
			if (retval < 0) {
				errmsg(retval);
