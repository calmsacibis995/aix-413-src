static char sccsid[] = "@(#)37	1.12.2.3  src/bos/usr/sbin/lvm/lreducelv.c, cmdlvm, bos411, 9428A410j 3/22/94 23:01:55";
 ...
 * FILE: lreducelv.c
 *
 * FILE DESCRIPTION: Source file for lreducelv program.  "lreducelv" provides
 *     a command level user interface to the lvm low level function lvm_reducelv.
 *
...
#define _ILS_MACROS	/* performance enhancement for isspace(), etc. calls */
#include <lvm.h>
#include <unistd.h>
#include <stdio.h>
#include <string.h>
...
static struct ext_redlv reducelv;  /* Record to pass to lvm_reducelv to perform the reduction */
...
int size;         /* size of the extention being added to the logical volume */
{
	register i;         /* indexes */
	...
	reducelv.parts = pp;	  /* the primary partition */
...
int Usage_error()
{
	fprintf(stderr, lvm_msg(RLV_USAGE), Prog);
}
 ...
 * NAME: main
 *
 * FUNCTION: lreducelv reads command line input from the user,
 *    parses it, performs basic validation, formats it, and calls
 *    lvm_reducelv passing the formatted values.  If an error is
...
char **argv;	   /* array of command line strings - from user */
{
	...
	/* The lvm reducelv record is filled by getmaps.		   */
	...
	if ((retval=ck_required()) == 0) {  /* ck_required confirms required parameters were supplied */
		...
		reducelv.size = atoi(s_value);     /* number of addition partitions */
		...
		/* call lvm to reduce the logical volume */
		if (retval = lvm_reducelv(&lv_id,&reducelv)) {
			errmsg(retval);
