#! /usr/bin/bsh
# @(#)36 1.18  src/bos/usr/sbin/acct/runacct.sh, cmdacct, bos411, 9428A410j 3/30/94 15:02:31
#
...
#set umask so that owner and group permissions do not get modified
umask 003
_MIN_BLKS=500
_wtmp=/var/adm/wtmp
...
if [ "$_blocks" -le $_MIN_BLKS ];then
	if MSG1=`dspmsg acct.cat 26 'runacct: Insufficient space on %1$s (%2$s blks); Terminating procedure\n' ${_fsdev} ${_blocks}`; then : 
	else MSG1="runacct: Insufficient space on ${_fsdev} (${_blocks} blks); Terminating procedure\n" ; fi
	echo $MSG1 >&2; echo $MSG1 | tee ${_active} | $MAILCOM
	rm -f ${_nite}/lock*
	exit 1
fi
...
*)
	...
	else echo "ERROR: runacct called with invalid arguments\n" > ${_active}r
	fi
	rm -f ${_nite}/lock*
	mv ${_active} ${_active}${_date}
	exit 1
...
do
	if test -r S${_i}.${_date} ; then
		...
		else echo "file setups probably already run\n" >> ${_active}; fi
		rm -f ${_nite}/lock*
		mv ${_active} ${_active}${_date}
		exit 1
	fi
...
if test -r ${_nite}/wtmp.${_date} ; then
	...
	if dspmsg acct.cat 36 "ERROR: S%s.%s already exists: run setup manually\n" ${_nite}/wtmp ${_date} > ${_active}; then :
	 	...
	 	echo "already exists: run setup manually\n" ) > ${_active}; fi
	rm -f ${_nite}/lock*
	mv ${_active} ${_active}${_date}
	exit 1
fi
#
#  acctcon1 expects the outstanding login sessions to be after the "runacct"
#  record.  The ut_time field of the "runacct" record will be used as the
#  "current" time when closing out the existing login sessions.
#
cp ${_wtmp} ${_nite}/wtmp.${_date}
acctwtmp "runacct" >> ${_nite}/wtmp.${_date}
fwtmp < /etc/utmp | awk '/^[^ ]/ && $4 == 7 {
        print $0
}' | sed -e 's/ 7 / 8 /' | fwtmp -ic >> ${_nite}/wtmp.${_date}
nulladm ${_wtmp}
#
#  acctcon1 will use the date of the "openacct" record to set the time in the
#  ut_time field for records which come physical later in the file, but which
#  have timestamps for an earlier time.
#
acctwtmp "openacct" >> ${_wtmp}
fwtmp < /etc/utmp | awk '/^[^ ]/ && $4 == 7 { print $0 }' | fwtmp -ic >> ${_wtmp}

if dspmsg acct.cat 37 "files setups complete\n" >> ${_active}; then :
else echo "files setups complete\n" >> ${_active}; fi
...
else echo "wtmp processing complete\n" >> ${_active}; fi 
...
acctcon2 < ctmp 2> log | acctmerg > ctacct.${_date}
# if the following test fails it means that acctcon2 failed
# to convert a ctmp record.  this is typically caused by running
# out of filesystem space.
if test -s log ; then
        df /usr/adm >> ${_active}${_date}
        cat log >> ${_active}${_date}
        echo "${_errormsg}" | $MAILCOM
        exit 1
fi

if dspmsg acct.cat 40 "connect acctg complete\n" >> ${_active}; then :
else echo  "connect acctg complete\n" >> ${_active}; fi
...
if dspmsg acct.cat 44 "all process actg complete for %s\n" ${_date} >> ${_active}; then :
else ( echo "all process actg complete for \c" 
...
cd ${_nite}
cp ctacct.${_date} daytacct
for i in ptacct*.${_date}
do
	acctmerg daytacct <$i >tmpdayt

	if [ $? -ne 0 ]
	then
		if dspmsg acct.cat 69  "An error occurred while executing the acctmerg program.\nThe pacct files will be restored in order to prevent the loss of accounting\ndata and all intermediate lock files will be removed.\n" 
		then :
		else echo "An error occurred while executing the acctmerg program.\nThe pacct files will be restored in order to prevent the loss of accounting\ndata and all intermediate lock files will be removed.\n"; fi
		cd ${_adm}
		...
		do
			if [ -f $i ]
			then
				mv $i `echo $i | sed "s/S//" | sed "s/\.[0-9]*//"` 
			fi
		done 
		rm -f /var/adm/acct/nite/*.${_date}
		rm -f /var/adm/acct/nite/lock*
		rm -f /var/adm/acct/nite/lastdate
		exit 1
	else
		mv daytacct daytacct.old
		mv tmpdayt daytacct
		rm -f daytacct.old
	fi
done
...
then :
else echo "tacct merge to create daytacct complete\n" >> ${_active}; fi
...
if test -s dacct; then
	...
	else echo "merged disk records\n" >> ${_active}; fi
	rm -f tmpdayt dacct
else
	if dspmsg acct.cat 49 "no disk records\n" >> ${_active}; then :
	else echo "no disk records\n" >> ${_active}; fi
fi
...
else echo "command summaries complete\n" >> ${_active}; fi
