static char sccsid[] = "@(#)99	1.7.1.1  src/bos/usr/sbin/rrestore/restore.c, cmdarch, bos411, 9428A410j 10/21/93 12:26:05";
/*
	...
	int type;
{
	...
	if (BIT(ino, dumpmap) == 0) {
		vprintf(stdout, MSGSTR(NOTONARCH, "%s is not on the archive\n"), name);
		return (descend);
	}
	if (!mflag) {
...
removeoldleaves()
{
	...
	vprintf(stdout, MSGSTR(MARKREM, "Mark entries to be removed.\n"));
...
#		define MODECHG	0x8	/* mode of inode changed */
	...
	case INOFND:
		...
		if (ip->e_type == NODE) {
			descend = FAIL;
			fprintf(stderr,
				MSGSTR(DELLN, "deleted hard link %s to directory %s\n"),
				name, myname(ip));
			break;
		}
...
findunreflinks()
{
	...
	vprintf(stdout, MSGSTR(FINDUNRE, "Find unreferenced names.\n"));
...
removeoldnodes()
{
	...
	vprintf(stdout, MSGSTR(REMOLD, "Remove old nodes (directories).\n"));
	do	{
		change = 0;
		prev = &removelist;
	...
	char *symtabfile;
{
	...
	if (command == 'R') {
		vprintf(stdout, MSGSTR(CANEXT, "Continue extraction of new leaves\n"));
	} else {
		vprintf(stdout, MSGSTR(EXTNEW, "Extract new leaves.\n"));
	...
	while (curfile.ino < maxino) {
		...
		if (first != curfile.ino) {
			fprintf(stderr, MSGSTR(EXPECF, "expected next file %d, got %d\n"),
				first, curfile.ino);
			skipfile(1);
			goto next;
		}
		...
		if (ep == NIL)
			panic(MSGSTR(UNKFILE, "unknown file on archive\n"));
		if ((ep->e_flags & (NEW|EXTRACT)) == 0)
			badentry(ep, MSGSTR(UNEXPECF, "unexpected file on archive"));
		/*
...
createfiles()
{
	...
	vprintf(stdout, MSGSTR(EXTREQ, "Extract requested files\n"));
	curfile.action = SKIP;
	getvol((long)1);
	skipmaps();
	skipdirs();
	...
	for (;;) {
		...
		while (next < curfile.ino) {
			...
			if (ep == NIL)
				panic(MSGSTR(CORSYM, "corrupted symbol table\n"));
			fprintf(stderr, MSGSTR(NOTONARCH,
				"%s is not found on the archive\n"), myname(ep));
			ep->e_flags &= ~NEW;
			next = lowerbnd(next);
		}
		/*
...
createlinks()
{
	...
	vprintf(stdout, MSGSTR(ADDLN, "Add links\n"));
	for (i = ROOTINO; i < maxino; i++) {
		ep = lookupino(i);
		if (ep == NIL)
			continue;
...
checkrestore()
{
	...
	vprintf(stdout, MSGSTR(CHECKSYM, "Check the symbol table.\n"));
	for (i = ROOTINO; i < maxino; i++) {
	...
	int type;
{
	...
	ep = lookupname(name);
	if (ep == NIL) {
		fprintf(stderr, MSGSTR(MISSNAM, "Warning: missing name %s\n"), name);
		return (FAIL);
	}
	np = lookupino(ino);
	if (np != ep)
