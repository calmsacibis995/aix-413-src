static char sccsid[] = "@(#)51	1.4  src/tcpip/usr/lib/libisode/prnt.c, isodelib7, tcpip411, 9437B411a 9/13/94 16:44:17";
/*
 * COMPONENT_NAME: (ISODELIB7) ISODE Libraries, Release 7
 *
 * FUNCTIONS: NEXT_PTPE VNAME VPRINT VSTRING VTAG VUNKNOWN dmp_ptpe next_ptpe 
 *    p_ismatch p_pr_choice p_pr_etype p_pr_obj p_pr_seq p_pr_seqof p_pr_set 
 *    p_pr_setof p_pr_type p_setpresent printable prnt_f setpval
 *
 * ORIGINS: 60
 *
 * FILE:	src/tcpip/usr/lib/libisode/prnt.c
 */

/* prnt.c */

#ifndef	lint
static char *rcsid = "$Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/prnt.c,v 1.3 93/02/11 11:40:58 snmp Exp $";
#endif

/* 
 * $Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/prnt.c,v 1.3 93/02/11 11:40:58 snmp Exp $
 *
 *
 * $Log:	prnt.c,v $
 * Revision 1.3  93/02/11  11:40:58  snmp
 * print snmpifo -d problem - D69836
 * 
 * Revision 1.2  93/02/05  17:07:02  snmp
 * ANSI - D67743
 * 
...
int counter_or_gauge;   /* Keeps track if type is Counter or Gauge */
...
modtyp *mod;			/* Module it is from */
{
	...
	case OBJECT:
	    counter_or_gauge = 0;
	    if ((!strcmp(p->pe_typename, "counter")) ||
	        (!strcmp(p->pe_typename, "ticks")) ||
		(!strcmp(p->pe_typename, "gauge")))
	       counter_or_gauge=1;
	...
	case INTEGER:
		...
		if (counter_or_gauge) {  /* Print %u if Counter or Gauge */
		    VPRINT(("%u", i));
		    counter_or_gauge = 0;
		}
		else                    /* All other INTEGER types */
		    VPRINT(("%d", i));
	    }
	    break;
