static char sccsid[] = "@(#)09	1.3  src/tcpip/usr/lib/libisode/logger.c, isodelib7, tcpip411, GOLD410 4/5/93 13:59:11";
/*
 * COMPONENT_NAME: (ISODELIB7) ISODE Libraries, Release 7
 *
 * FUNCTIONS: _ll_log _ll_printf ll_check ll_close ll_dbinit ll_defmhdr 
 *    ll_hdinit ll_log ll_open ll_preset ll_printf ll_setmhdr ll_sync
 *
 * ORIGINS: 60
 *
 * FILE:	src/tcpip/usr/lib/libisode/logger.c
 */

/* logger.c - system logging routines */

#ifndef	lint
static char *rcsid = "$Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/logger.c,v 1.2 93/02/05 17:05:15 snmp Exp $";
#endif

/* 
 * $Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/logger.c,v 1.2 93/02/05 17:05:15 snmp Exp $
 *
 *
 * $Log:	logger.c,v $
 * Revision 1.2  93/02/05  17:05:15  snmp
 * ANSI - D67743
 * 
...
register LLog *lp;
{
    ...
    if (lp -> ll_file == NULLCP || *lp -> ll_file == NULLCHAR) {
you_lose: ;
	(void) ll_close (lp);
	lp -> ll_stat |= LLOGERR;
	return NOTOK;
...
char   *prefix;
{
    ...
    if ((lp -> ll_hdr = (char *)malloc ((unsigned) (strlen (buffer) + 1))) == NULLCP)
	return;
...
char   *prefix;
{
	...
	if ((lp -> ll_file = (char *)malloc ((unsigned) (strlen (buffer) + 1)))
	        == NULLCP)
	    return;
