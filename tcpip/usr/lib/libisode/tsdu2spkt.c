static char sccsid[] = "@(#)02	1.3  src/tcpip/usr/lib/libisode/tsdu2spkt.c, isodelib7, tcpip411, GOLD410 4/5/93 17:22:58";
/*
 * COMPONENT_NAME: (ISODELIB7) ISODE Libraries, Release 7
 *
 * FUNCTIONS: If_Reset If_Set Put_Item Put_MData Put_Ref Put_SSN Put_UData 
 *    Put_XData Reset Set advance end_pgi end_spdu freespkt newspkt pullqb 
 *    put2spdu spkt2tsdu start_pgi start_spdu str2ssn tsdu2spkt
 *
 * ORIGINS: 60
 *
 * FILE:	src/tcpip/usr/lib/libisode/tsdu2spkt.c
 */

/* tsdu2spkt.c - read/write a SPDU to a TSDU */

#ifndef	lint
static char *rcsid = "$Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/tsdu2spkt.c,v 1.2 93/02/05 17:14:16 snmp Exp $";
#endif

/* 
 * $Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/lib/libisode/tsdu2spkt.c,v 1.2 93/02/05 17:14:16 snmp Exp $
 *
 *
 * $Log:	tsdu2spkt.c,v $
 * Revision 1.2  93/02/05  17:14:16  snmp
 * ANSI - D67743
 * 
...
int basesize;
{
    ...
    if ((c -> top = (char *)malloc ((unsigned) c -> len)) == NULL) {
	c -> len = 0;
...
*/
	char   *cp;
	    ...
	    if (c -> allocli < 255)
		cl += 2;
	    cp = (char *)realloc (c -> top, (unsigned) (c -> len += cl));
	    if (cp == NULL) {
		c -> len = 0;
		return;
	    }
...
int	n;
{
	...
	if (buffer == NULL) {
	    if ((buffer = (char *)malloc ((unsigned) SEGMENT_MAX)) == NULL)
		return NULLCP;
	    cp = buffer;
	}
...
do_pgi: ;
		...
		s -> s_udata = (char *)malloc ((unsigned) (s -> s_ulen = li));
		if (s -> s_udata == NULL) {
		    s -> s_errno = SC_CONGEST;
		    break;
		}
		...
		switch (si) {
		    case SPDU_RF: 
			s -> s_rdata = (char *)malloc ((unsigned) (s -> s_rlen = li));
			if (s -> s_rdata == NULL) {
			    s -> s_errno = SC_CONGEST;
			    break;
			}
