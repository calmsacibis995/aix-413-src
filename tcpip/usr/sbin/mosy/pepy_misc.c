static char sccsid[] = "@(#)34	1.6  src/tcpip/usr/sbin/mosy/pepy_misc.c, isodelib7, tcpip411, GOLD410 10/29/93 10:04:27";
/*
 * COMPONENT_NAME: (ISODELIB7) ISODE Libraries, Release 7
 *
 * FUNCTIONS: addoid addtable addtableref check_impexp defineoid free_expimp 
 *    initoidtbl int2oid oidlookup oidname oidprint print_expimp
 *
 * ORIGINS: 27  60
 *
 ...
 *
 * FILE:	src/tcpip/usr/sbin/mosy/pepy_misc.c
 */

/* pepy_misc.c - PE parser (yacc-based) misc routines */

#ifndef	lint
static char *rcsid = "$Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/sbin/mosy/pepy_misc.c,v 1.2 93/02/08 09:58:15 snmp Exp $";
#endif

/*
 * $Header: /vikings/u/snmp/projects/harriet/RCS/src/tcpip/usr/sbin/mosy/pepy_misc.c,v 1.2 93/02/08 09:58:15 snmp Exp $
 *
 *
 * $Log:	pepy_misc.c,v $
 * Revision 1.2  93/02/08  09:58:15  snmp
 * ANSI - D67743
 * 
...
OID	o1, o2;
{
    ...
    if (noid == NULLOID)
	myyerror (MSGSTR(MS_GENERAL, GENERA_3, "Out of Memory (%d needed)"),
		  sizeof(*noid));
...
OID	oid;
{
    ...
    if (oid == NULLOID) {
	myyerror (MSGSTR(MS_INFO, INFO_40, "Warning Null oid in defineoid"));
	return;
    }
	...
	if (strcmp (op -> op_name, name) == 0) {
		...
		warning (MSGSTR(MS_INFO, INFO_41, 
...
char	*name;
{
    ...
    warning (MSGSTR(MS_INFO, INFO_42, "unknown Object Identifier '%s'"), name);
    return NULLOID;
}
...
YP	yp;
{
	...
	if (strcmp (sp -> sym_name, yp -> yp_identifier) == 0) {
		...
		myyerror (MSGSTR(MS_INFO, INFO_43, 
 ...
 * NAME: free_expimp
 *
 * FUNCTION: Free export and import tables.
 */
free_expimp (table)
int table;
{
