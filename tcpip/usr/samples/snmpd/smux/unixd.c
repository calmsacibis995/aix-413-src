static char sccsid[] = "@(#)93	1.4  src/tcpip/usr/samples/snmpd/smux/unixd.c, snmp, tcpip411, GOLD410 3/29/94 15:24:45";
/*
 * COMPONENT_NAME: (SNMP) Simple Network Management Protocol Daemon
 *
 * FUNCTIONS: main adios advise arginit doit_smux envinit get_smux getkmem 
 *    init_unix mibinit o_mbuf o_mbufType set_smux start_smux
 *
 * ORIGINS: 60
 *
 * FILE:	/usr/samples/snmpd/smux/unixd.c
 */
 ...
 *
 * $Log:	unixd.c,v $
 * Revision 1.2  93/02/08  09:51:56  snmp
 * ANSI - D67743
 * 
...
#ifdef _POWER
    if (knlist (nl, sizeof (nl)/sizeof (struct nlist), sizeof (struct nlist)) == NOTOK)
#else
    if (nlist ("/vmunix", nl) == NOTOK)
#endif
	adios (UNIX, "unable to nlist");
...
{
    if (n -> n_value == 0) {
	advise (LLOG_EXCEPTIONS, NULLCP, "\"%s\" not in %s", 
		n -> n_name, UNIX);
	return NOTOK;
    }
