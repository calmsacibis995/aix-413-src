static char sccsid[] = "@(#)88	1.1  src/rspc/usr/lib/methods/common/cfgisadev.c, isatok, rspc411, GOLD410 5/3/94 14:08:56";
/*
...
/*
 *   cfgisadev.c - Generic Configure Method Code
...

{
    ...
    if (errflg) {
	    ...
	    DEBUG_0("cfgisadev: command line error\n");
	    exit(E_ARGS);
    }
    ...
    if (logical_name == NULL) {
	    DEBUG_0("cfgisadev: logical name must be specified\n");
	    exit(E_LNAME);
    }

    DEBUG_1 ("cfgisadev: Configuring device: %s\n",logical_name)

    /* start up odm */
    if (odm_initialize() == -1) {
	    /* initialization failed */
	    DEBUG_0("cfgisadev: odm_initialize() failed\n")
	    exit(E_ODMINIT);
    }
    DEBUG_0 ("cfgisadev: ODM initialized\n")

    /* open customized devices object class */
    if ((int)(dev_obj.cusdev = odm_open_class(CuDv_CLASS)) == -1) {
	    DEBUG_0("cfgisadev: open class CuDv failed\n");
	    err_exit(E_ODMOPEN);
    }
	...
	DEBUG_1("cfgisadev: failed to find CuDv object for %s\n", logical_name);
	err_exit(E_NOCuDv);
    }
    else if (rc==-1) {
	/* ODM failure */
	DEBUG_0("cfgisadev: ODM failure getting CuDv object");
	err_exit(E_ODMGET);
    }

    /* open predefined devices object class */
    if ((int)(dev_obj.predev = odm_open_class(PdDv_CLASS)) == -1) {
	    DEBUG_0("cfgisadev: open class PdDv failed\n");
	    err_exit(E_ODMOPEN);
    }
    ...
    if (rc==0) {
	    ...
	    DEBUG_0("cfgisadev: failed to find PdDv object for this device\n");
	    err_exit(E_NOPdDv);
    }
    else if (rc==-1) {
	    /* ODM failure */
	    DEBUG_0("cfgisadev: ODM failure getting PdDv object");
	    err_exit(E_ODMGET);
    }
    ...
    DEBUG_2("cfgisadev: name of parent device = %s,rc=%d\n",dev_obj.cusobj.parent,rc);
    if (rc==0)
    {
	    /* Parent device not in CuDv */
	    DEBUG_0("cfgisadev: no parent CuDv object\n");
	    err_exit(E_NOCuDvPARENT);
    }
    else if (rc==-1)
    {
	    /* ODM failure */
	    DEBUG_0("cfgisadev: ODM failure getting parent CuDv object\n")
	    err_exit(E_ODMGET);
    }
	...
	{
		DEBUG_0("cfgisadev: parent is not AVAILABLE")
	...
	else if (rc)
	{
		...
		DEBUG_0("cfgisadev: device already AVAILABLE at this connection\n")
	 ...
	 ***************************************************/
	if (!strcmp(dev_obj.preobj.subclass,"isa"))
	{
	    if (ipl_phase == RUNTIME_CFG)
	    {
#ifdef NEED_CHG
		DEBUG_0("cfgisadev: checking adapter for slot and bus resources\n")
		...
		if (rc != 0) {
			DEBUG_2("cfgisadev: card %s not found in slot %d\n",
				logical_name,slot);
			err_exit(rc);
		}
#endif
		...
		*/
		...
		if (rc == 0)
		{
		    ...
		    if (rc != 0)
		    {
			DEBUG_0("cfgisadev: bus resources could not be resolved\n")
			err_exit(rc);
		    }
#endif
		}
		...
		{
		    DEBUG_0("cfgisadev: device not on bus\n")
		}
		else
		{
		    DEBUG_0("cfgisadev: ODM err getting parent bus\n")
		    err_exit(rc) ;
		}
	...
	DEBUG_1("cfgisadev: checking for DvDr entry, DvDr=%s\n",dev_obj.preobj.DvDr);
	if (strcmp(dev_obj.preobj.DvDr, "") != 0)
	{
	    ...
	    if (inst_num == -1) {
		DEBUG_0("cfgisadev: error getting instance number\n")
		err_exit(E_INSTNUM);
	    }
	    DEBUG_1("cfgisadev: instance number = %d\n",inst_num);

	    DEBUG_0("cfgisadev: load a device driver\n")
	    /* call loadext to load the device driver */
	    if ((cfg_k.kmid = loadext(dev_obj.preobj.DvDr, TRUE, FALSE))
		 == NULL)
	    {
		/* error loading device driver */
		DEBUG_1("cfgisadev: error loading driver %s\n", dev_obj.preobj.DvDr)
		err_exit(E_LOADEXT);
	    }
	    dev_kmid = cfg_k.kmid;
	    DEBUG_1("cfgisadev: kernel module id = %x\n",dev_kmid)
	    ...
	    /* build the DDS  */
	    DEBUG_0("cfgisadev: Calling build_dds()\n")
	    rc = build_dds(logical_name, &dev_obj,
			   &ndd_config.dds,
			   &l_dds );
	    if (rc) {
		    /* error building dds */
		    DEBUG_1("cfgisadev: error building dds, rc=%d\n",rc)
		    err_undo1(dev_obj.preobj.DvDr);
		    if ( rc < 0 || rc > 255)
			    rc = E_DDS;
		    err_exit(rc);
	    }
	    DEBUG_0("cfgisadev: Returned from build_dds()\n")

	    /* call sysconfig to pass DDS to driver */
	    DEBUG_0("cfgisadev: Pass DDS to driver via sysconfig()\n")
	    cfg_k.cmd = CFG_INIT;
	    if (sysconfig(SYS_CFGKMOD, &cfg_k, sizeof(struct cfg_kmod )) == -1) {
		    /* error configuring device */
		    DEBUG_0("cfgisadev: error configuring device\n")
		    err_undo1(dev_obj.preobj.DvDr);
		    err_exit(E_CFGINIT);
	    }

	    /* download microcode if necessary */
	    DEBUG_0("cfgisadev: Calling download_microcode()\n")
		...
		DEBUG_1("cfgisadev: error downloading microcode, rc=%d\n",rc)
	    ...
	    DEBUG_0("cfgisadev: Returned from download_microcode()\n")
		...
		if ( conferrno != E_NOPdOBJ ) {
			...
			DEBUG_0("cfgisadev: error getting addl dvdr name\n")
		...
		DEBUG_2("cfgisadev: calling %s %s\n",addl_dvdr, sstring)

		if(odm_run_method(addl_dvdr,sstring,NULL,NULL)){
			fprintf(stderr,"cfgisadev: can't run %s\n",
				addl_dvdr);
			err_undo2(dev_kmid,inst_num);
			err_undo1(dev_obj.preobj.DvDr);
	...
	if (dev_obj.preobj.has_vpd == TRUE) {
	    ...
	    if (rc) {
		    /* failed to get VPD */
		    DEBUG_1("cfgisadev: error getting VPD, rc=%d\n",rc)
		    err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
		    if ( rc < 0 || rc > 255)
			    rc = E_VPD;
		    err_exit(rc);
	    }
	    DEBUG_0("cfgisadev: Returned from query_vpd()\n")

	    /* open customized vpd object class */
	    if ((int)(cusvpd = odm_open_class(CuVPD_CLASS)) == -1) {
		    DEBUG_0("cfgisadev: open class CuVPD failed");
		    err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
		    err_exit(E_ODMOPEN);
	    }
	    ...
	    if (rc==-1) {
		    /* ODM failure */
		    DEBUG_0("cfgisadev: ODM failure getting CuVPD object");
		    err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
		    err_exit(E_ODMGET);
	    }
	    if (rc==0) {
		/* need to add vpd object */
		DEBUG_0("cfgisadev: Adding new VPD object\n");
		strcpy(vpdobj.name,logical_name);
		vpdobj.vpd_type = HW_VPD;
		memcpy(vpdobj.vpd,vpd,sizeof(vpd));
		if (odm_add_obj(cusvpd,&vpdobj) == -1) {
			DEBUG_0("cfgisadev: ODM failure adding CuVPD object")
			err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
			err_exit(E_ODMADD);
		}
		DEBUG_0("cfgisadev: Successfully added new VPD object\n");
	    } else {
		/* see if vpd object needs to be updated */
		if (memcmp(vpdobj.vpd,vpd,sizeof(vpd))) {
		    DEBUG_0("cfgisadev: Updating VPD object\n");
		    memcpy(vpdobj.vpd,vpd,sizeof(vpd));
		    if (odm_change_obj(cusvpd,&vpdobj) == -1) {
			DEBUG_0("cfgisadev: ODM failure updating CuVPD object")
			err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
			err_exit(E_ODMUPDATE);
		    }
		    DEBUG_0("cfgisadev: Successfully updated VPD object\n");
		}
	    }
	    /* close customized vpd object class */
	    if (odm_close_class(CuVPD_CLASS) == -1) {
		    DEBUG_0("cfgisadev: error closing CuVPD object class\n");
		    err_undo3(dev_obj.preobj.DvDr, dev_kmid, inst_num);
		    err_exit(E_ODMCLOSE);
	    }
	...
	{
	    /* ODM failure */
	    DEBUG_0("cfgisadev: ODM failure updating CuDv object\n");
	    err_exit(E_ODMUPDATE);
	}
...
char    *DvDr;                  /* pointer to device driver name */
{
	...
	if (loadext(DvDr,FALSE,FALSE) == NULL) {
		DEBUG_0("cfgisadev: error unloading driver\n");
	}
	return;
}
...
int     inst_num_2;             /* The device's instance number */
{
	...
	if (sysconfig(SYS_CFGKMOD,&cfg_k,sizeof(struct cfg_kmod)) == -1) {
		DEBUG_0("cfgisadev: error unconfiguring device\n");
	}
