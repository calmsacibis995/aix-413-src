static char sccsid[] = "@(#)64	1.3  src/rspc/usr/lib/methods/probe_ibmtok/probe_ibmtok.c, isatok, rspc411, 9438C411a 9/23/94 12:58:01";
/*
...
/* This probe is specifically for Token Ring for adapter/isa/tokenring	*/
...
#define		THIS_TYPE  	TOKEN_BOOT

#define		PNPID	  0x2783d041	
#define		DEVUT	  "adapter/isa/tokenring"
#define		DTYPE	  "tokenring"
...
int		interrupt;	/* Read from adapter			*/
int		bios_address;	/* Read from adapter			*/
int		boot_device;	/* Is this the boot device		*/
int		ring_speed;	/* Token ring speed			*/
...
if (iplcb_get(&iplcb_dir,128,sizeof(iplcb_dir),MIOIPLCB)) 
	...
	exit(1);
					...
					/* Get ring_speed from net_data */

if (iplcb_get(&net_data,iplcb_dir.net_boot_results_offset,sizeof(net_data),MIOIPLCB)) 
	{
	perror("Error reading net_data from IPL Control block");
	exit(1);
        }

if(net_data.is_802_3)
	ring_speed = 16;
else
	ring_speed = 4;
					/* It's variable size 		*/
...
for (i=0; i<rp->ActualNumDevices; i++) 
	...
	if(rp->Devices[i].DeviceId.DevId == PNPID)
		...
		port_val = get_isa_port(io_address, bus_name);
...
if(foundit && (port_val != 0xFF))	/* Found in residual and valid port	*/
	{
	bios_address = 0xC0000 +((port_val & 0x3C) << 11);

	interrupt = (port_val & 0x2) ? ((port_val & 0x1) ? 7 : 6) :
       	    (port_val & 0x1) ? 3 : 9;

	DEBUG_2("BIOS Address = 0x%X  Interrupt = %d\n",
		 bios_address, interrupt);
...
sprintf(sstring,"0x%x",bios_address);
strcpy(cuat->value,sstring);
if((putattr(cuat)) == -1)
	{
	DEBUG_2("error updating %s attr for %s\n",gattr,lname)
