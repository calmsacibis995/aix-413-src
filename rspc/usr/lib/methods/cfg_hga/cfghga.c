static char sccsid[] = "@(#)40  1.8  src/rspc/usr/lib/methods/cfg_hga/cfghga.c, pcihga, rspc411, 9434A411a 8/20/94 17:40:21";
/*
 *
...
/*
 * Local include for hga dds structure
 */
#include <hgadds.h>
#include <hga_reg.h>
        ...
        char    *bus_name;
{
        ...
        DEBUG_0 ("cfg_hga: calling odm_open_class PdDv\n")

        if (((int) (p_Class_PdDv = odm_open_class (PdDv_CLASS))) == FAIL)
        {
                DEBUG_1 ("cfg_hga: open class PdDv failed rc=%d.\n", rc)
                odm_close_class (p_Class_CuDv);
                return(E_ODMOPEN);
        }

        DEBUG_1 ("cfg_hga: calling odm_get_first() for device %s.\n",
                 logical_name )
        ...
        if ((rc == FAIL) || (rc == NULL))
        {
                DEBUG_2 ("cfg_hga: Get CuDv failed rc=%d 0x%x.\n", rc, rc )
                odm_close_class (p_Class_CuDv);
        ...
        DEBUG_1 ("cfg_hga: searching PdDv for  %s\n",
        ...
        if ((rc == FAIL) || (rc == NULL))
        {
                DEBUG_1 ("cfg_hga: failed to find PdDv object rc=%d.\n", rc)
                odm_close_class (p_Class_CuDv);
                odm_close_class (p_Class_PdDv);
                return(E_ODMGET);
        }
...
ulong           default_color_table [16] = {
        ...
        *size = sizeof(struct fbdds);
        ...
        {
            DEBUG_0("cfg_hga: malloc failed for dds structure");
            return(E_MALLOC);
        }
        ...
        if (odm_initialize () == FAIL)
        {
            ...
            DEBUG_0 ("cfg_hga: odm_initialize() failed\n")
            return (E_ODMINIT);
        }

        if (((lock_id = odm_lock ("/etc/objrepos/config_lock", 0)) == FAIL))
        {
            DEBUG_0 ("cfg_hga: odm_lock() failed\n")
            return (E_ODMLOCK);
        }

        DEBUG_0 ("cfg_hga: ODM initialized and locked\n")
        ...
        if(rc == 0)
        {
            ...
            DEBUG_1("cfg_hga: device id =0x%8.8x\n",dds->devid);

            dds->slot_number = strtoul(where,NULL,10);

            DEBUG_1("cfg_hga: dds->slot_number = %d\n", dds->slot_number);

            DEBUG_1("cfg_hga: bus_name =%s\n",bus_name);
        }
        else
	{
            DEBUG_1 ("cfg_hga: get_ODM_info rc=0x%x\n",rc);
	}
        ...
        if(rc == 0)
        {
            ...
            {
                dds->bid = value;
                DEBUG_1("cfg_hga: bid = 0x%8.8x\n", dds->bid);
            }
            else 
            {   
                DEBUG_0 ("cfg_hga: ODM attr: bus_id missing\n");
#ifdef BUS_ID
                dds->bid = BID_VAL(IO_PCI, PCI_IOMEM, 0);
                rc = 0;
                DEBUG_1 ("cfg_hga: assuming bid = [%x]\n", dds->bid);
#endif
            }
        }
        if(rc == 0)
        {
                    ...
                    != NULL)
            {
                ...
                DEBUG_1("cfg_hga: bus_mem_address = 0x%8.8x\n", dds->bus_mem_address);
                dds->IO_address = 0; /* maybe someday this will be */
                                     /* relocatable too            */
            }
            else 
	    {
		DEBUG_0 ("cfg_hga: ODM attr: bus_mem_start missing\n");
	    }
        }
        ...
        if(rc == 0)
        {
                    ...
                    != NULL)
            {
                ...
                DEBUG_1("cfg_hga: dds->display_mode = 0x%8.8x\n", dds->display_mode);
            }
            else 
	    {
		DEBUG_0 ("cfg_hga: ODM attr: display_mode missing\n");
        ...
        if(rc == 0)
        {
            if((rc = get_instance(lname, &value)) == E_OK )
            {
                ...
                if (putattr(cusatt) < 0)
                {
                    DEBUG_0("cfg_hga: putattr failed\n");
                    rc = E_ODMUPDATE;
                }
            }
            else 
	    {
		DEBUG_0 ("cfg_hga: get_instance failed\n");
	    }
        }
        ...
        if(rc == 0)
        {
            ...
            if( get_attrval(lname, "belongs_to", temp, &value, &rc) != NULL )
            {
                ...
                {
                    DEBUG_2("cfg_hga: Couldn't add dependency for %s -> %s\n", 
			temp, lname);
                }
                else
                {
                    DEBUG_2("cfg_hga: Added dependency for %s -> %s\n", 
			temp, lname);
                }
            }
            else 
	    {
		DEBUG_0 ("cfg_hga: ODM attr: belongs_to missing\n");
	    }
        }
