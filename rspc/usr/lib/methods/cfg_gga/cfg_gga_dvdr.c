static char sccsid[] = "@(#)51  1.2  src/rspc/usr/lib/methods/cfg_gga/cfg_gga_dvdr.c, pcigga, rspc41J, 9516B_all 4/18/95 10:26:21";
/*
...

{
	...
	if (dvdr[0] != '\0') {
		...
		/* Construct the _subtype  */
		DEBUG_0("Calling build_subtype()\n")
		rc = build_subtype(cudv.name, cfg.ddsptr);
		if (rc) {
			/* error building _subtype */
			DEBUG_1("error building _subtype, rc=%d\n",rc)
			if ( rc < 0 || rc > 255)
				rc = E_DDS;
			err_exit(rc);
		}
	...
	rc = device_specific(cudv.name,cudv.location); 
	if (rc) {
		/* error returned */
		err_exit(rc);
	}
