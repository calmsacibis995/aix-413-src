static char sccsid[] = " @(#)60        1.1  src/rspc/usr/lib/methods/cfgdgric_isa/cfgdgric_isa.c, dd_artic, rspc41J, 9508A_notx 2/17/95 16:02:23";
/*
...
#include <cf.h>
#include <fcntl.h>
#include <dartic_isa/dartic_isa.h>
#include <dartic_isa/darticdd_isa.h>
...
extern  char    *optarg;        /* for getopt function */
        ...
        if (cusobj.status == DEFINED) {
                ...
                if ((articdds.basetype = getbasetype(&parobj)) == -1)
                   err_exit(E_NODETECT);
                ...
                articdds.windowsize = 0x2000; /* 8K window size */
 ...
 *
 *  INPUTS:
 *      parobj  - Pointer to parents CuDv object
 *
...
struct CuDv *parobj;
{
        ...
        /* Determine base type from parent's CuDv object */
        if (strstr(parobj->name, "ampx") != NULL)
           return(MULTIPORT);

        if (strstr(parobj->name, "apm") != NULL)
           return(GALE);
