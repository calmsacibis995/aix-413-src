static char sccsid[] = "@(#)83  1.3  src/rspc/kernext/pcmcia/ent/ient_init.c, pcmciaent, rspc41J, 9511A_all 3/13/95 00:02:50";
/*
static char sccsid[] = "@(#)74  1.7.1.1  src/rspc/kernext/isa/ient/ient_init.c, isaent, rspc41J 1/6/95 15:46:58";
*/
/*
...
ient_probe(dds_t  *p_dds)
{
  ...
  WRK.slot_width = 1;
                             ...
                             IEN_ISA_AT_DETECT_REG_OFFSET)) & 0x01))
    {
        ...
        if ((read_back & 0x03) == io_base_bits)
        {
...
ient_start(dds_t *p_dds)    /* dev_ctl area ptr */
{
...
#ifdef PCMCIAENT
    *(ioaddr + DDI.io_addr + PAGE0_TPSR) = PACKET_BUFFER_ADDRESS >> 8;
#else
    ...
    *(ioaddr + DDI.io_addr + PAGE0_PSTART) = IEN_ISA_NBR_TX_PAGES;
#ifdef PCMCIAENT
  *(ioaddr + DDI.io_addr + PAGE0_PSTOP) = (DDI.shared_mem_size /
      IEN_ISA_PAGE_SIZE) + (PACKET_BUFFER_ADDRESS >> 8);
#else
    *(ioaddr + DDI.io_addr + PAGE0_PSTOP) = (DDI.shared_mem_size / 
                                             IEN_ISA_PAGE_SIZE);
#endif
    ...
    *(ioaddr + DDI.io_addr + PAGE0_BNRY) = IEN_ISA_NBR_TX_PAGES;
    eieio();
    ...
    *(ioaddr + DDI.io_addr + PAGE1_CURR) = IEN_ISA_NBR_TX_PAGES + 1;
    eieio();
...
ient_stop_NIC(dds_t *p_dds, volatile uchar *ioaddr)
{
    ...
    /* This sequence of events is prescribed to stop the NIC. */

    /* Stop the NIC. */
    cmd.value = IEN_ISA_NO_DMA | IEN_ISA_STOP_NIC;
    *(ioaddr + DDI.io_addr + PAGE0_COMMAND) = cmd.value;
    eieio();
    ...
    *(ioaddr + DDI.io_addr + PAGE0_RBCR1) = 0;
	...
	io_delay( 1000 );
#endif
        reset_entered = *(ioaddr + DDI.io_addr + PAGE0_ISR);
        eieio();
    } while ( cnt++ < 10 && !(reset_entered & IEN_ISA_RESET_PENDING));

#ifdef PCMCIAENT
    TRACE_SYS(HKWD_IEN_ISA_OTHER, "nixE", (ulong)p_dds, (ulong)reset_entered, 0);
#else
    TRACE_SYS(HKWD_IEN_ISA_OTHER, "nixE", (ulong)p_dds, 0, 0);
#endif
    return(0);
}
