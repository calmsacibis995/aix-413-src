static char sccsid[] = "@(#)68  1.6  src/rspc/kernext/pcmcia/tok/itok_rw.c, pcmciatok, rspc41J, 9511A_all 3/9/95 02:39:01";
/*
static char sccsid[] = "@(#)49	1.9  src/rspc/kernext/isa/itok/itok_rw.c, isatok, rspc41J 1/6/95 15:59:01";
 */
/*
...
#ifdef PCMCIATOK
    if( tx_elem == NULL || tx_elem->in_use == 0 ){
      goto end_tok_ssb_response;
    }
#endif
...
#ifdef PCMCIATOK
    long     mbuf_left;                     /* mbuf space left in mbufp      */
#endif
...
#ifdef PCMCIATOK
    TRACE_DBG(HKWD_ITOK_ISA_OTHER,"OUTb",(ulong)WRK.xmits_queued,
          (ulong)WRK.tx_buf_in_use,(ulong)p_mbuf->m_pkthdr.len);

    if( !p_dds->configured ){
        TRACE_DBG(HKWD_ITOK_ISA_OTHER, "OUT+", 0, 0, 0);
        return(ENETUNREACH);
    }
#endif
