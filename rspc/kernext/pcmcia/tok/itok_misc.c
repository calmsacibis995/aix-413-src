static char sccsid[] = "@(#)63  1.4  src/rspc/kernext/pcmcia/tok/itok_misc.c, pcmciatok, rspc41J, 9511A_all 3/9/95 02:37:51";
/*
static char sccsid[] = "@(#)47	1.7  src/rspc/kernext/isa/itok/itok_misc.c, isatok, rspc41J 9/21/94 14:27:07";
 */
/*
...
/*****************************************************************************/
/*
 * NAME:                  tok_stop_timers
 *
 * FUNCTION:              stop timers and release blocked threads.
 *                        tokclose is assumed to be called later.
 *
 * EXECUTION ENVIRONMENT: PCMCIA CS event handler
 *
 * NOTES:
 *
 * RETURNS:               nothing
 ...
 */
/*****************************************************************************/
void tok_stop_timers(dds_t   *p_dds)  /* Device structure ptr. */
{
  int sap;
  int   rc;                  /* Return code.                             */
  int   event;               /* What cmd/event timed out ?               */
  ...
  /* wait for the wrong interrupts completion */
  pri = disable_lock( PL_IMP, &SLIH_LOCK );
  unlock_enable( pri, &SLIH_LOCK );

  pri = disable_lock( PL_IMP, &TX_LOCK );

  if( WRK.adap_state != CLOSED_STATE && WRK.xmits_queued ){
