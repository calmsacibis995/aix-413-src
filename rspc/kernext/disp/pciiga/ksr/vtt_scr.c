static char sccsid[] = "@(#)69	1.3  src/rspc/kernext/disp/pciiga/ksr/vtt_scr.c, pciiga, rspc41J, 9515B_all 3/22/95 17:12:36";
/*
 *
...
BUGLPR(dbg_vttscr,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  IGA_ENTER_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR);
      ...
      (ld->Vttenv.vtt_mode != KSR_MODE)
  {
     ...
     IGA_EXIT_TRC1(ddf,iga_vttscr,2,IGA_VTTSCR,EIO);
  ...
  ********************************/
  ...
  if((lines >= ld->Vttenv.ps_size.ht) || (lines <= -ld->Vttenv.ps_size.ht))
  {
    ...
    if (ld->Vttenv.vtt_active == VTT_ACTIVE)
    {
      ...
      IGA_PARM_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR,IGA_VTTSCR_CLR_WHOLE_FB);
    ...
    IGA_EXIT_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR);
    return(0);
  }
  else
  {
    ...
    else /* Terminal is active so scroll screen and update row lengths */
    {
      ...
      if (lines < 0)
      {
        IGA_PARM_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR,IGA_VTTSCR_CLR_PART_FB_FOR_SCRL_DOWN);
      ...
      else
      {
        ...
        IGA_PARM_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR,IGA_VTTSCR_CLR_PART_FB_FOR_SCRL_UP);
        ...
        (ld->Vttenv.cursor_show = cursor_show /* assignment */)
      {
        ...
        IGA_PARM_TRC0(ddf,iga_vttscr,2,IGA_VTTSCR,IGA_VTTSCR_CALL_MOVC);
       ...
       num_rows;      /* number of rows to copy     */
{
  ...
  *         This loop updates the the IGA VRAM                        *
  *********************************************************************
  ********************************************************************/
