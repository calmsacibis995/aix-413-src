static char sccsid[] = "@(#)63	1.4  src/rspc/kernext/disp/pciiga/ksr/vtt_cpl.c, pciiga, rspc41J, 9515B_all 3/22/95 17:10:03";
/*
 *
...
ulong  cursor_show; /* if true cursor is moved to */
{
  ...
  ddf = (struct iga_data *) vp->display->free_area;
  ld = (struct iga_data *) vp->vttld;

  IGA_ENTER_TRC0(ddf,iga_vttcpl,2,IGA_VTTCPL);

  VDD_TRACE(CPL , ENTRY, vp);

  if (ld->Vttenv.vtt_mode != KSR_MODE)  /* If not in character mode */
  { 
  	IGA_EXIT_TRC1(ddf,iga_vttcpl,2,IGA_VTTCPL,EIO);
	return(EIO);
  }
...
BUGLPR(dbg_vttcpl,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  {
    IGA_PARM_TRC0(ddf,iga_vttcpl,2,IGA_VTTCPL,IGA_VTTCPL_UPDATE_FB);
  ...
  {
    IGA_PARM_TRC0(ddf,iga_vttcpl,2,IGA_VTTCPL,IGA_VTTCPL_CALL_MOVC);
  ...
  IGA_EXIT_TRC0(ddf,iga_vttcpl,2,IGA_VTTCPL);
