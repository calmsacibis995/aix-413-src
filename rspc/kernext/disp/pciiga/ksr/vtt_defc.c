static char sccsid[] = "@(#)65	1.3  src/rspc/kernext/disp/pciiga/ksr/vtt_defc.c, pciiga, rspc41B, 9504A 12/15/94 17:01:47";
/*
 *
...
ulong  cursor_show;   /* 0 ==> don't show the cursor */
{
  ...
  IGA_ENTER_TRC0(ddf,iga_vttdefc,2,IGA_VTTDEFC);
  ...
  { /* Only valid in character (KSR) mode */
    ...
    IGA_EXIT_TRC1(ddf,iga_vttdefc,2,IGA_VTTDEFC,EIO);
...
BUGLPR(dbg_vttdefc,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  ******************************************/
  ...
  { /* if the virtual terminal is active, update adapter's cursor shape */
    /* otherwise, igaact() will update the shape with with the selector */
...
BUGLPR(dbg_vttdefc,BUGGID,("  shape = default\n"));
    ...
    IGA_PARM_TRC0(ddf,iga_vttdefc,2,IGA_VTTDEFC, IGA_VTTDEFC_CHANGE_CURSOR);
  ...
  { /* the cursor must be moved */
    IGA_PARM_TRC0(ddf,iga_vttdefc,2,IGA_VTTDEFC, IGA_VTTDEFC_CALL_MOVC);
    vttmovc(vp);
  }
  VDD_TRACE(DEFC , EXIT, vp);
  /*BUGLPR(dbg_vttdefc,BUGNTA,("<DEFC ret(0)\n"));*/

  IGA_EXIT_TRC0(ddf,iga_vttdefc,2,IGA_VTTDEFC);
...
/*                                                                     */
/* FUNCTION:  This routine sets the IGA local data environment cursor  */
...
/*                    igadefc() if the selected cursor shape           */
...
BUGLPR(dbg_vttdefc,BUGGID,("  bottom = %d\n",bottom));
  ...
  { if (ld->Vttenv.cursor.data != NULL)
    ...
    { iga_err(vp,"PCIIGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.data,IGA_BAD_XMALLOC,
       RAS_UNIQUE_3);
      return(ENOMEM);
    }
    ...
    { iga_err(vp,"PCIIGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.ichar,IGA_BAD_XMALLOC,
       RAS_UNIQUE_3);
      return(ENOMEM);
    }
