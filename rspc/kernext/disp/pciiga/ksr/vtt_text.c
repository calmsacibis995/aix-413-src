static char sccsid[] = "@(#)73	1.3  src/rspc/kernext/disp/pciiga/ksr/vtt_text.c, pciiga, rspc41J, 9515B_all 3/22/95 17:14:00";
/*
 *
                                 ...
                                 /* the pos given in cp_parms    */
{
  ...
  IGA_ENTER_TRC0(ddf,iga_vtttext,2,IGA_VTTTEXT);

  if (ld->Vttenv.vtt_mode != KSR_MODE)/* not in character mode */
  { /* Only valid in character (KSR) mode */
    IGA_EXIT_TRC1(ddf,iga_vtttext,2,IGA_VTTTEXT,EIO);
    return(EIO);
  }
...
BUGLPR(dbg_vtttext,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  if (ld->Vttenv.vtt_active) /* the virtual terminal is active */
  {
    ...
    IGA_PARM_TRC0(ddf,iga_vtttext,2,IGA_VTTTEXT,IGA_VTTTEXT_CALL_DRAW_TEXT);
  ...
  BUGLPR(dbg_vtttext,BUGNTA,("Moving the cursor from vtt_text\n"));
    IGA_PARM_TRC0(ddf,iga_vtttext,2,IGA_VTTTEXT,IGA_VTTTEXT_CALL_MOVC);
    vttmovc(vp);   /* Call movc to move the cursor */
  }

  VDD_TRACE(TEXT , EXIT, vp);
  BUGLPR(dbg_vtttext,BUGNTA,("Leaving VTT TEXT\n"));
  IGA_EXIT_TRC0(ddf,iga_vtttext,2,IGA_VTTTEXT);
  return(0);
} /* end of vtttext */
