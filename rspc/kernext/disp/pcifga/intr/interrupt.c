static char sccsid[] = "@(#)11  1.2  src/rspc/kernext/disp/pcifga/intr/interrupt.c, pcifga, rspc411, GOLD410 4/21/94 18:32:37";
/* interrupt.c */
/*
 ...
 */
    ...
    /*----------------------------------------------------------*
     |								|
     |   THIS CODE IS AN EXAMPLE, FGA DOESN'T USE INTERRUPTS.	|
     |								|
     *----------------------------------------------------------*/

/*--------------------------------------------------------------------
 |
 | IDENTIFICATION:  FGA_INTR
 |
 | DESCRIPTIVE NAME:  FGA_INTR - Interrupt handler for Frame Buffer
 |
 | FUNCTIONS:   fga_intr
 |
...
fga_intr (intdat)
        struct intr    *intdat;
{
#ifdef FW_INTERRUPTS	/* Fairway not currently supporting interrupts. */
	...
	ld->bus_mem_addr = PCI_MEM_ATT(&ddf->pci_mem_map);
        ...
        if (int_set)
        {
                ...
                /* We have received a fga interrupt */
                ...
                if (ddf->cmd & EVENT_MODE)
                {
                        ...
                        if (ddf->cmd & SYNC_REQ || ddf->cmd & ASYNC_REQ)
                        {
                                ...
                                ddf->report.wa = 0;
                                ddf->report.data[0] = 0;
                                ddf->report.data[1] = int_set;    /* FGA Interrupt Status */
                                ddf->report.data[2] = int_enabled;/* FGA Interrupts
                                                                   * Currently Enabled */
                                ddf->report.data[3] = 0;
                ...
                if (ddf->cmd & DIAG_MODE)
                {
                        ...
                        if (int_set & WTK_VBLANK_SET) /*only one used by FGA */
                                ddf->diaginfo[7]++;
                }
                ...
                {
                        /* FGA Don't Know DMA */
                        BUGLPR (dbg_intr, BUGGID, ("FGAdd Sent DMA Wait Interrupt CMD!!!!!!\n"));
                }
                else
                if (ddf->cmd & DMA_IN_PROGRESS)
                {
                        /* FGA Don't Know DMA */
                        BUGLPR (dbg_intr, BUGGID, ("FGAdd Sent DMA Interrupt CMD!!!!!!\n"));
                }
