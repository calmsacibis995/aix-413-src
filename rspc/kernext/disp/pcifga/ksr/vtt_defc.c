static char sccsid[] = "@(#)26  1.1  src/rspc/kernext/disp/pcifga/ksr/vtt_defc.c, pcifga, rspc411, GOLD410 4/15/94 18:41:54";
/* vtt_defc.c */
/*
...
BUGLPR(dbg_vttdefc,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  ******************************************/
  ...
  { /* if the virtual terminal is active, update adapter's cursor shape */
    /* otherwise, fgaact() will update the shape with with the selector */
...
/*                                                                     */
/* FUNCTION:  This routine sets the FGA local data environment cursor  */
...
/*                    fgadefc() if the selected cursor shape           */
                ...
                bottom;   /* top and bottom line numbers in cursor */
{
  ...
  { if (ld->Vttenv.cursor.data != NULL)
    ...
    if (ld->Vttenv.cursor.data == NULL)
    { fga_err(vp,"PCIFGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.data,FGA_BAD_XMALLOC,
       RAS_UNIQUE_1);
      return(ENOMEM);
    }
    ...
    { fga_err(vp,"PCIFGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.ichar,FGA_BAD_XMALLOC,
       RAS_UNIQUE_2);
      return(ENOMEM);
    }
