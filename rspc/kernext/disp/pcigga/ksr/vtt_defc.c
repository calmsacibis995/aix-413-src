static char sccsid[] = "@(#)13  1.2  src/rspc/kernext/disp/pcigga/ksr/vtt_defc.c, pcigga, rspc41B, 9504A 1/9/95 13:35:37";
/* vtt_defc.c */
/*
based on "@(#)53  1.4  src/bos/kernext/disp/wga/ksr/vtt_defc.c, wgadd, bos410, 9404A410a 1/19/94 16:45:49";
 *
...
BUGLPR(dbg_vttdefc,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  ******************************************/
  ...
  { /* if the virtual terminal is active, update adapter's cursor shape */
    /* otherwise, ggaact() will update the shape with with the selector */
    ...
    GGA_PARM_TRC0(ddf,gga_vttdefc,2,GGA_VTTDEFC, GGA_VTTDEFC_CHANGE_CURSOR);
  ...
  { /* the cursor must be moved */
    GGA_PARM_TRC0(ddf,gga_vttdefc,2,GGA_VTTDEFC, GGA_VTTDEFC_CALL_MOVC);
    vttmovc(vp);
  }
  VDD_TRACE(DEFC , EXIT, vp);
  /*BUGLPR(dbg_vttdefc,BUGNTA,("<DEFC ret(0)\n"));*/

  GGA_EXIT_TRC0(ddf,gga_vttdefc,2,GGA_VTTDEFC);
...
/*                                                                     */
/* FUNCTION:  This routine sets the GGA local data environment cursor  */
...
/*                    ggadefc() if the selected cursor shape           */
                ...
                bottom;   /* top and bottom line numbers in cursor */
{
  ...
  { if (ld->Vttenv.cursor.data != NULL)
    ...
    if (ld->Vttenv.cursor.data == NULL)
    { gga_err(vp,"PCIGGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.data,GGA_BAD_XMALLOC,
       RAS_UNIQUE_1);
      return(ENOMEM);
    }
    ...
    { gga_err(vp,"PCIGGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.ichar,GGA_BAD_XMALLOC,
       RAS_UNIQUE_2);
      return(ENOMEM);
    }
