static char sccsid[] = "@(#)21  1.3  src/rspc/kernext/disp/pcigga/ksr/vtt_text.c, pcigga, rspc41J, 9513A_all 3/22/95 11:22:01";
/* vtt_text.c */
/*
                                 ...
                                 /* the pos given in cp_parms    */
{
  ...
  GGA_ENTER_TRC0(ddf,gga_vtttext,2,GGA_VTTTEXT);

  if (ld->Vttenv.vtt_mode != KSR_MODE)/* not in character mode */
  { /* Only valid in character (KSR) mode */

    GGA_EXIT_TRC1(ddf,gga_vtttext,2,GGA_VTTTEXT,EIO);
...
BUGLPR(dbg_vtttext,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  if (ld->Vttenv.vtt_active) /* the virtual terminal is active */
  {
    ...
    GGA_PARM_TRC0(ddf,gga_vtttext,2,GGA_VTTTEXT,GGA_VTTTEXT_CALL_DRAW_TEXT);
  ...
  BUGLPR(dbg_vtttext,BUGNTA,("Moving the cursor from vtt_text\n"));
    GGA_PARM_TRC0(ddf,gga_vtttext,2,GGA_VTTTEXT,GGA_VTTTEXT_CALL_MOVC);
  ...
  GGA_EXIT_TRC0(ddf,gga_vtttext,2,GGA_VTTTEXT);
