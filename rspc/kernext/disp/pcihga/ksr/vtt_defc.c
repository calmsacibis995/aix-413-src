static char sccsid[] = "@(#)46  1.2  src/rspc/kernext/disp/pcihga/ksr/vtt_defc.c, pcihga, rspc411, 9440A411e 10/6/94 15:21:25";
/* vtt_defc.c */
/*
...
BUGLPR(dbg_vttdefc,BUGGID,("cursor_show       = %d\n",cursor_show));
  ...
  ******************************************/
  ...
  { /* if the virtual terminal is active, update adapter's cursor shape */
    /* otherwise, hgaact() will update the shape with with the selector */
...
/*                                                                     */
/* FUNCTION:  This routine sets the HGA local data environment cursor  */
...
/*                    hgadefc() if the selected cursor shape           */
...
BUGLPR(dbg_vttdefc,BUGGID,("  bottom = %d\n",bottom));
  ...
  { if (ld->Vttenv.cursor.data != NULL)
    ...
    { hga_err(vp,"PCIHGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.data,HGA_BAD_XMALLOC,
       RAS_UNIQUE_3);
      return(ENOMEM);
    }
    ...
    { hga_err(vp,"PCIHGA","VTT_DEFC","XMALLOC",ld->Vttenv.cursor.ichar,HGA_BAD_XMALLOC,
       RAS_UNIQUE_3);
      return(ENOMEM);
    }
