static char sccsid[] = "@(#)55  1.2  src/rspc/kernext/disp/pcihga/intr/interrupt.c, pcihga, rspc411, GOLD410 4/21/94 17:35:45";
/* interrupt.c */
/*
...
/*--------------------------------------------------------------------
 |
 | IDENTIFICATION:  HGA_INTR
 |
 | DESCRIPTIVE NAME:  HGA_INTR - Interrupt handler for Frame Buffer
 |
 | FUNCTIONS:   hga_intr
 |
        ...
        struct intr    *intdat;
{
...
#if S3_INTERRUPTS /* when HGA does interrupts */
	...
	ld->PCI_BUS_ATT(&ddf->pci_io_map);
        ...
        if (int_set)
        {
                ...
                /* We have received a hga interrupt */
                ...
                if (ddf->cmd & EVENT_MODE)
                {
                        ...
                        if (ddf->cmd & SYNC_REQ || ddf->cmd & ASYNC_REQ)
                        {
                                ...
                                ddf->report.wa = 0;
                                ddf->report.data[0] = 0;
                                ddf->report.data[1] = int_set;/* HGA Interrupt Status */
                                ddf->report.data[2] = 0;
                                ddf->report.data[3] = 0;
                ...
                {
                        /* HGA Don't Know DMA */
                        BUGLPR (dbg_intr, BUGGID, ("HGAdd Sent DMA Wait Interrupt CMD!!!!!!\n"));
                }
                else
                if (ddf->cmd & DMA_IN_PROGRESS)
                {
                        /* HGA Don't Know DMA */
                        BUGLPR (dbg_intr, BUGGID, ("HGAdd Sent DMA Interrupt CMD!!!!!!\n"));
                }
