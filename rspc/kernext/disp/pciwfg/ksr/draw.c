static char sccsid[] = "@(#)10  1.1  src/rspc/kernext/disp/pciwfg/ksr/draw.c, pciwfg, rspc411, 9434B411a 8/24/94 07:43:06";
/* draw.c */
/*
 *
 ...
 *
 * DESCRIPTIVE NAME:  color expand and output to characters to WFG VRAM
 *
 *
		...
		*FB_PIXEL(cur_x, cur_y) = (uchar)segbpp8;               \
        ...
        {                                                               \
		       ...
		       ("(fg%X, bg%X)\n", ld->fgcol, ld->bgcol));       \
                ...
                |------------------------------------------*/           \
                                                                        \
                temps = (volatile ushort *)FB_PIXEL(cur_x, cur_y);      \
		*temps = (ushort)segbpp8;                               \
                                                                        \
        ...
        {                                                               \
            ...
            {                                                           \
		       ...
		       ("(fg%X, bg%X)\n", ld->fgcol, ld->bgcol));       \
                ...
                |--------------------------------------------------*/   \
                                                                        \
                templ = (volatile ulong *)FB_PIXEL(cur_x, cur_y);       \
		*templ = segbpp8;                                       \
                                                                        \
 ...
 *
 * DESCRIPTIVE NAME: Draw a character to the wfg adapter.
 *
 * CALLED BY: Draw_text
 *
 * CALLS:     colex()
...

{
	...
	ddf = (struct wfg_ddf *) vp->display->free_area;
        ...
        indexptr = (ulong)(aixCharInfoPtr) ld->fontcindex[char_offset].byteOffset;
                ...
                if (boxw >= WORDLEN)
                {
                        ...
                        for (wth = 0; wth < fullseg; wth++)
                        {
                                ...
                                if (ld->reverse_video)
                                        segbpp1 ^= 0xFFFFFFFF;
                                /*
                                 * and color expand that data segment and
                                 * write it to WFG VRAM
                                 */
                        ...
                        xstart;
{
	...
	ddf = (struct wfg_ddf *) vp->display->free_area;
        ...
        BUGLPR (dbg_clrlines, BUGGID, ("ld->bpp =%x\n",ld->bpp));
        pre_pels = xstart & 3;
        tmp = WORDLEN / ld->bpp;
        ...
        BUGLPR (dbg_clrlines, BUGGID, ("linelen =0x%8x\n",linelen));
        ...
        /*-------------------------------------------------------------
         ...
         -------------------------------------------------------------*/
                ...
                if(pre_pels > 1)
                {
                        ...
                        temps = (volatile ushort *)FB_PIXEL(xpos, topline);
                        *temps = (ushort)bgword;
                        xpos += 2;
                ...
                for (; xpos < linelen; xpos+=advance)
                {
                        ...
                        templ = (volatile ulong *)FB_PIXEL(xpos, topline);
                        *templ = bgword;
                }
                if(remppl > 1)
                {
        ...
        PCI_BUS_DET(ld->bus_base_addr);
        BUGLPR (dbg_clrlines, BUGNTA, ("dettaching PCI Bus\n"));
}
/*-------------------------------------------------------------------------
 *
 * DESCRIPTION OF MODULE:
...
char            draw_blanks;
{
        ...
        if (constant_attributes)
        {
                ...
                for (j = current_character; j < string_length; j++)
                {
                        ...
                        if (first_blank)
                        {
                                 ...
                                 * blanks
                                 */
                BUGLPR (dbg_drawtxt, BUGGID, ("PRE Clear lines.\n"));
				clear_lines (vp, ld,
                ...
                BUGLPR (dbg_drawtxt, BUGGID, ("POST Clear lines.\n"));
                                first_blank = 0;
                        }
                        cur_x++;
                }
        ...
        {
                BUGLPR (dbg_drawtxt, BUGGID, ("Not constant attributes.\n"));
