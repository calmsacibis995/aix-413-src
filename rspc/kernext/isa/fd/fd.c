#ifndef lint
static char sccsid[] = "@(#)64 1.22 src/rspc/kernext/isa/fd/fd.c, isafd, rspc41J, 9518A_all 5/1/95 16:01:08";
#endif
/*
 ...
 *		fd_qvpd_exit 		
 *		fdconfig_motor_start
 *           	fdconfig_motor_stop 	
 *		fdconfig_recalibrate 
 *		fdconfig_reset		
 *		fdconfig_seek
 *		fdconfig_sense_interrupt
 *		fdconfig_soft_reset
 *           	fdconfig_specify 
 *		fdconfig_vpd 
 *		fddoor_check 
...
int	fd_config (dev_t devno, int cmd, register struct uio *uiop)
{
	...
	case CFG_TERM:
		...
		if (minor_number & FD_ADAP_MINOR_NUM) {
			...
			/* unregister the power management handler */
   			DEBUG_0 ("fd_config: pwr mgt unregister\n");
			pm_register_handle (&fdadapter->pwr_mgt, PM_UNREGISTER);
			...
			/* unregister the power management handler */
   			DEBUG_0 ("fd_config: pwr mgt unregister\n");
			pm_register_handle (&fdp->pwr_mgt, PM_UNREGISTER);
	...
	case CFG_INIT:
		...
		{  /* configure the diskette adapter */
			...
			DEBUG_1 ("fd_config: init adapter, addr = %x\n", 
								fdadapter);
			/* copy in dds */
			...
			fdadapter->int_class     = (int) configptr->
							fda.int_class;
			...
			for (i = 0; i < 8; i++) 
				fdadapter->logical_name[i] = 
					configptr->fda.adap_name[i];

			/* init values needed for PIO */
			fdadapter->iom.key     = IO_MEM_MAP;
			fdadapter->iom.flags   = 0;
			fdadapter->iom.size    = SEGSIZE;
			fdadapter->iom.bid     = configptr->fda.bus_id;
			fdadapter->iom.busaddr = 0;

			/* init values for power management */
			fdadapter->pwr_mgt.activity         = PM_NO_ACTIVITY;
			fdadapter->pwr_mgt.mode             = PM_DEVICE_FULL_ON;
			fdadapter->pwr_mgt.idle_counter     = 0;
			fdadapter->pwr_mgt.handler          = fd_pmh;
			fdadapter->pwr_mgt.private          = 
							(caddr_t) fdadapter;
			fdadapter->pwr_mgt.devno            = devno;
			fdadapter->pwr_mgt.device_idle_time = 
					configptr->fda.device_idle_time;
			fdadapter->pwr_mgt.device_standby_time = 
					configptr->fda.device_standby_time;
			fdadapter->pwr_mgt.attribute        = 
						configptr->fda.pm_attribute;
			fdadapter->pwr_mgt.device_logical_name = 
						fdadapter->logical_name;
			fdadapter->pwr_mgt.device_idle_time1 = 0;
			fdadapter->pwr_mgt.device_idle_time2 = 0;
			fdadapter->pwr_mgt.reserve[0]        = '\0';
			fdadapter->pwr_mgt.reserve[1]        = '\0';
			fdadapter->pwr_mgt.pm_version        = PM_PHASE2_SUPPORT;
			fdadapter->pwr_mgt.extension         = 0;

			rc = pm_register_handle (&fdadapter->pwr_mgt, PM_REGISTER);
			...
			if (rc != 0) {
				pm_register_handle (&fdadapter->pwr_mgt, 
						    PM_UNREGISTER);
				rc = free((caddr_t)fdadapter);
				ASSERT(rc == 0);
				fdadapter = NULL;
			...
			lock_alloc(&fdadapter->intr_lock, 
				LOCK_ALLOC_PIN, FDDD_LOCK_CLASS, -1);
			simple_lock_init(&fdadapter->intr_lock);
		...
		{ /* configure a drive instead of adapter */
		  ...
		  /* - copy data into floppy structure    */
			...
			if (fdadapter->drive_list[drive_number] != NULL) {
		 	 ...
		 	 { /* if not, create floppy structure for drive */
				...
				fdp->last_error1 = 0;
				fdp->last_error2 = 0;
				fdp->pwr_mgt_block = FALSE;
				fdp->pwr_mgt_sleep_anchor = EVENT_NULL;
			} 
			...
			fdp->pm_devid = configptr->fdd.pm_device_id;
			...
			/* init values for power management */
			fdp->pwr_mgt.activity         = PM_NO_ACTIVITY;
			fdp->pwr_mgt.mode             = PM_DEVICE_FULL_ON;
			fdp->pwr_mgt.idle_counter     = 0;
			fdp->pwr_mgt.handler          = fd_pmh;
			fdp->pwr_mgt.private          = (caddr_t) fdp;
			fdp->pwr_mgt.devno            = devno;
			fdp->pwr_mgt.device_idle_time = 
					configptr->fdd.device_idle_time;
			fdp->pwr_mgt.device_standby_time = 
					configptr->fdd.device_standby_time;
			fdp->pwr_mgt.attribute        = 
						configptr->fdd.pm_attribute;
			fdp->pwr_mgt.device_logical_name = 
						fdp->resource_name;
			fdp->pwr_mgt.device_idle_time1 = 0;
			fdp->pwr_mgt.device_idle_time2 = 0;
			fdp->pwr_mgt.reserve[0]        = '\0';
			fdp->pwr_mgt.reserve[1]        = '\0';
			fdp->pwr_mgt.pm_version        = PM_PHASE2_SUPPORT;
			fdp->pwr_mgt.extension         = 0;

			rc = pm_register_handle (&fdp->pwr_mgt, PM_REGISTER);
			ASSERT (rc == PM_SUCCESS);
...
int	fd_open (dev_t devno, ulong devflag)
{
	...
	if (fdadapter->initialized == FALSE) {
		...
		/* Put driver in DUMMY state to accept an interrupt */
		/* immediately after "i_init"			    */
		if (fdadapter->first_open == TRUE) {
			fdadapter->state = FD_DUMMY_INTERRUPT;
		}
		fdadapter->fdhandler->handler = fd_intr;
		fdadapter->fdhandler->bus_type = BUS_BID;
		fdadapter->fdhandler->flags = INTR_MPSAFE | INTR_NOT_SHARED;
		fdadapter->fdhandler->level = fdadapter->bus_int_level;
		fdadapter->fdhandler->priority = INTCLASS3;
		fdadapter->fdhandler->bid = fdadapter->iom.bid;
		FD_TRACE1("i_init", devno);
		 ...
		 */
		DIO_INIT(&fdadapter->dio, 1);
		FD_TRACE1("dma init", &fdadapter->dio);
		fdadapter->dma_id = D_MAP_INIT(fdadapter->iom.bid, DMA_SLAVE,
		    0, fdadapter->dma_level);
		if ((int) fdadapter->dma_id == DMA_FAIL) {
			FD_TRACE1("new state", fdp->drive_state);
			fdp->drive_state = FDCLOSED;
			fdcleanup(drive_number);
			unlockl(&fd_lock);
...
int 	fdiocformat (long arg)
{
	...
	fdadapter->dio.dvec->iov_base = fdadapter->format_buffer;
	fdadapter->dio.dvec->iov_len = fdp->format_size;
	fdadapter->dio.dvec->xmp = &fdp->xmem;
	dma_rc = D_MAP_SLAVE (fdadapter->dma_id, 0, 
				fdp->format_size, &fdadapter->dio, 
	...
	if (rc != FDSUCCESS) {
		if (fdadapter->d_comp_needed == TRUE) {
			trc = D_UNMAP_SLAVE (fdadapter->dma_id);
		      ...
		      register union fd_config *vpdptr, uchar drive_number)
{
	...
	/* We need to know if a 2.88 MB or 1.44 MB diskette drive is 	    */
	/* present.  Unfortunately, the drive ID bits are not fully 	    */
	/* functional on Sandalfoot systems.  First thing to do is check    */
	/* if the bits are functional.  Thus, the following check needs to  */
	/* be performed:						    */
        /* Check if the media sense and drive id bits are valid.            */
        /* reading '1111 xxxx' from register 3f3 with no drive selected     */
	/* (motor off) will indicate they are valid.   			    */
        DEBUG_0 ("cfgvpd: fdconfig_drive_sense\n");
        rc = fdconfig_drive_sense (&piop);
        if (rc != FDSUCCESS) {
                fdadapter->reset_needed = TRUE;
                FD_BUSIO_DET(piop.bus_val);
                FD_TRACE1("out cfgvpd", FDP_DEV);
        ...
        DEBUG_1 ("drive sense data = %x\n", piop.data);
	check_drive = FALSE;
        if ((piop.data & 0xf0) == 0xf0 && drive_number == 0)
		check_drive = TRUE;
	 ...
	 * If the drive ID bits are operational determine if it is
	 * a 3.5" 2.88 MB or 1.44 MB
	 */
	DEBUG_0 ("cfgvpd: fdconfig_drive_sense\n");
	rc = fdconfig_drive_sense (&piop);
	if (rc != FDSUCCESS) {
	...
	DEBUG_1 ("drive sense data = %x\n", piop.data);
	if ((check_drive == TRUE) && ((piop.data & 0x30) == 0x10))
...
int	fdconfig_reset ( register struct pio_parms *piop )
{
	...
	/* activate the RESET bit on the controller */
	mask = FDNOINTRESETMASK;
	piop->reg = FDDRIVE_CONTROL_REG;
	piop->read = FALSE;
	piop->data = mask;
	rc = fdcfg_cntl_pio ((caddr_t)(piop));

	if (rc == 0) {
		/* delay until the controller has a chance to RESET */
		curtime(&start_time);
		...
		/* deactivate the RESET bit on the controller */
		mask = mask | 0x04 | FDNODRIVE_SELECT;
		piop->data = mask;
		rc = fdcfg_cntl_pio ((caddr_t)(piop));
		if (rc == 0) {
			fdadapter->reset_needed = FALSE;
	    ...
	    register struct pio_parms *piop )
{
	...
	rc = fdcfg_data_pio ((caddr_t)(piop));
	return(rc);
}
...
/* This routine reads the media sense/drive ID data */
/* to determine the type of drive present           */
int     fdconfig_drive_sense ( register struct pio_parms *piop )
{
        int     rc;

        piop->reg = FDMEDIA_SENSE_REG;
        piop->read = TRUE;
        piop->data = 0;
        rc = fdcfg_cntl_pio ((caddr_t)(piop));
        return(rc);
