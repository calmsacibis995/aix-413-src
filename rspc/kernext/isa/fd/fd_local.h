/* @(#)99 1.4 src/rspc/kernext/isa/fd/fd_local.h, isafd, rspc41J, 9513A_all 3/28/95 13:52:38 */
#ifndef _H_FD_LOCAL
#define _H_FD_LOCAL
...
#define FDNODRIVE_SELECT 0x03	/* value of drive select bits with no */
				/* drive running		      */
				/* This value is used so that the media type */
				/* may be detected without illuminating the  */
				/* active drive light on rspc machines       */
...
#define	FDMEDIA_SENSE_REG	3	/* read only  */
...
#define FD_BUSIO_ATT(iomstruct, io_addr)   ((void *) \
		              ((ulong) iomem_att(&(iomstruct)) + io_addr))
#define FD_BUSIO_DET(ioatt_addr)           iomem_det(ioatt_addr)
...
struct floppy {
	struct	pm_handle pwr_mgt;	/* Power Management structure */
...
#define	FDUNKNOWN	0
	...
	uchar	motor_off_time;	/* inactive time before motor turned off 
					(seconds) */
	uchar  	pwr_mgt_block;  /* is a process blocked due to */
				/* PM suspend or hibernate?    */
	...
	int	pm_devid;	/* the drive's PM planar device id */
	int	pwr_mgt_sleep_anchor;	/* event word for power mgt lock */
...
struct adapter_structure {
	...
	int	int_class;	/* diskette interrupt class */
	...
	char	*speed_buffer;	/* pointer to buffer for speed and settle
					ioctls */
	d_handle_t  dma_id;		/* DMA channel id */
	struct	io_map iom;		/* PIO I/O structure            */
	struct	dio    dio;		/* DMA I/O structure            */
	...
	char   logical_name[8];		/* the device name of the adapter */
#ifdef DEBUG
