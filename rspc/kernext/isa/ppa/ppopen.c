#ifndef lint
static char sccsid[] = "@(#)51 1.4 src/rspc/kernext/isa/ppa/ppopen.c, sysxprnt, rspc41J, 9510A 3/6/95 23:23:08";
#endif
/*
...
int ext)          /* extension val     */
{
	...
	if (pp->pmh != NULL) {
		...
		case PM_DEVICE_HIBERNATION:
			pp->pmblock = TRUE;
			/* the system is about to suspend/hibernate 	*/
			/* e_wakeup will be called after resume		*/
			e_sleep(&(pp->pppmblock), EVENT_SHORT);
		}
		pp->pmbusy = TRUE;
#ifdef	_POWER_MP
		...
		i_enable(opri);
#endif	/* _POWER_MP */
		pm_planar_control(dev, PMDEV_PARALLEL, PM_PLANAR_ON);
	}
#endif	/* PM_PHASE2 */
...
#ifdef	PM_PHASE2
	if ((retcd != 0) && (pp->pmh != NULL)) {
		pp->pmbusy = FALSE;
		pm_planar_control(dev, PMDEV_PARALLEL, PM_PLANAR_OFF);
	}
#endif	/* PM_PHASE2 */
    }
