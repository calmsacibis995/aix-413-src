#ifndef lint
static char sccsid[] = "@(#)50 1.4 src/rspc/kernext/isa/ppa/ppconfig.c, sysxprnt, rspc41J, 9510A 3/6/95 23:22:27";
#endif
/*
				...
				if (retcd == 0) {
					...
					if (pp == (struct pp *)NULL) {
						...
						if (pp->pmh != NULL) {
							...
							i_enable(opri);
#endif	/* _POWER_MP */
							pm_planar_control(devno, PMDEV_PARALLEL, PM_PLANAR_ON);
						}
#endif	/* PM_PHASE2 */
						retcd = ppinit_dev(pp) ;
#ifdef	PM_PHASE2
						if (pp->pmh != NULL) {
							pp->pmbusy = FALSE;
							pm_planar_control(devno, PMDEV_PARALLEL, PM_PLANAR_OFF);
						}
#endif	/* PM_PHASE2 */
						if (retcd) {
							ppfree (pp) ;
