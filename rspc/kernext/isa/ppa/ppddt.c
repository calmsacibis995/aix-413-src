#ifndef lint
static char sccsid[] = "@(#)07	1.36.1.1  src/rspc/kernext/isa/ppa/ppddt.c, sysxprnt, rspc41J, 9510A 3/6/95 23:22:54";
#endif
/*
...
#ifdef	PM_PHASE2
void	pppm_init(struct pp *);
void	pppm_term(struct pp *);
int	pppm_handler(caddr_t, int);
#endif	/* PM_PHASE2 */
...
void
pppm_init(struct pp *pp)
{
	int	rc;

	pp->pmh = (struct pm_handle *)xmalloc(sizeof (struct pm_handle), 3, pinned_heap);
	if (pp->pmh != NULL) {
		pp->pmh->activity		= PM_NO_ACTIVITY;
		pp->pmh->mode			= PM_DEVICE_FULL_ON;
		pp->pmh->device_idle_time	= 0;	/* better to extract from */
		pp->pmh->device_standby_time	= 0;	/* ODM via config method  */
		pp->pmh->idle_counter		= 0;
		pp->pmh->handler		= pppm_handler;
		pp->pmh->private		= (caddr_t)pp;
		pp->pmh->devno			= pp->dev;
		pp->pmh->attribute		= 0;
		pp->pmh->device_idle_time1	= 0;
		pp->pmh->device_idle_time2	= 0;
		pp->pmh->device_logical_name	= &(pp->ppelog.header.resource_name[0]);
		pp->pmh->pm_version		= PM_PHASE2_SUPPORT;
		pp->pmh->extension		= NULL;
		rc = pm_register_handle(pp->pmh, PM_REGISTER);
		if (rc != PM_SUCCESS) {
			xmfree(pp->pmh, pinned_heap);
			pp->pmh = NULL;
		}
	}
	pp->pmbusy	= FALSE;
	pp->pmblock	= NULL;
	pp->pppmblock	= EVENT_NULL;
	/* pp->pmdatareg = 0; */
	/* pp->pmctrlreg = 0; */
}

void
pppm_term(struct pp *pp)
{
	int	rc;

	if (pp->pmh != NULL) {
		rc = pm_register_handle(pp->pmh, PM_UNREGISTER);
		xmfree(pp->pmh, pinned_heap);
		pp->pmh = NULL;
	}
}
